...
- **別プロジェクトで作業中に harness を直したくなっても、その作業リポジトリの中で直さない。** 開発用 clone の場所は環境変数 `CLAUDE_HARNESS_DEV_DIR` で解決する。未設定なら主に聞く（`find` 等の探索で当てにいかない）。そこから worktree を切り、別セッションかサブエージェントに任せる
...
- 変更したら `plugin.json` のバージョンを上げ、commit・push する（作業ツリーに置きっぱなしにしない）

詳細（編集禁止の理由、開発用 clone からの worktree の切り方、マージ前の `--plugin-dir` での動作確認、マージ後の反映手順、状態が壊れたときの復旧、旧運用からの移行）は `~/.claude/plugins/marketplaces/oratta-claude-harness/docs/worktree-recovery.md`。
