...
- [x] 3.1 `plugins/dev-workflow/skills/pr-review-gate/SKILL.md` 手順 2-1 の「姿勢」の後に、レビュアー向け指示ブロック（固定書式・深刻度の定義表・全件列挙の 1 文・再レビューの制限・直し方どおりの箇所を再指摘しない）を貼り付け可能な形で置く
- [x] 3.2 1.2 の判定に従う: 適用されない（または未実測・どちらとも言えない）なら対応表は置かない。適用されるなら、手順 2-1 に Codex の JSON（`priority` / `confidence_score` / `code_location`）から固定書式への対応表を置き、`confidence_score` を `confirmed` の代わりにしないことを添える
