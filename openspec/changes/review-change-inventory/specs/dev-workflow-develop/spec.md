...
G が `review-incomplete` を return したとき、本体は新しい reviewer を起動せず、`agent-review:pending` のまま残差を報告して工程を止めなければならない（MUST）。`needs-reviewer` を受けたときは同じレビューの固定 HEAD・元の三表・残差・補足済み回数を fresh reviewer に渡して不足分だけを補わせ、レビューを最初からやり直させてはならない（MUST NOT）。
