...
The new checker implements the common first-stage operation for both grep consumers: validate the common metadata/columns and compare the fixed-revision search hits with all table rows regardless of the fourth-column value. The grep-term path of triage step 3 alone retains its second stage against post-fix HEAD, including matched text, multiplicity, and deleted-line checks. First-pass reconciliation compares one fixed SHA only and does not perform that post-fix stage.

The case-axis path of triage step 3 stays outside this common search contract. It keeps the existing `検索コマンド:` description of the axis and its full domain plus the two-column `| 軸の値 | 扱い |` table. G closes that path by comparing the table rows with the full axis domain only; it does not require a grep command, the four common columns, or the post-fix second stage.
...
If a Codex response uses its priority-shaped output and omits the three tables, the existing interpretation table treats the omitted artifacts as incomplete reviewer output and applies the same supplement path. If a difference remains with `補足済み回数: 1`, G records the residual and returns terminal Status `review-incomplete`. The implementation adds that value to the Status enumeration in `gate-runner.md` and to the G-return branches in develop `SKILL.md` (4). The coordinator does not launch another reviewer for that status, leaves `agent-review:pending`, reports the residual, and does not enter the pass path. `needs-reviewer` is therefore only a request below the limit; `review-incomplete` is the exhausted fail-closed outcome.
