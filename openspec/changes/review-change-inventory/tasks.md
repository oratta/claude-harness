...
- [ ] 2.1 `plugins/dev-workflow/skills/pr-review-gate/SKILL.md` の順 3 の grep の語の経路と一周目レビュアー向け `text` ブロックが参照する共通一覧契約を 1 か所に置く。`修正前 SHA: <40桁>`、repository-wide な `git grep -n <語の指定> <rev> -- .`、`| ファイル | 行（修正前 SHA） | ヒットした行の本文 | 扱い |` を定め、追跡対象パスの除外・検索起点の限定を禁止する。順 3 の場合分けの軸は既存どおり `検索コマンド:` に軸と全域、表に `| 軸の値 | 扱い |` を使い、共通検索契約の対象外とする
- [ ] 2.2 同ブロックに、`変更点の一覧`・`照合表`・`ハンク被覆` の固定見出しと、`変更点の一覧 → 照合表 → ハンク被覆 → 自己点検 → 指摘` の順序を追加する。変更点ごとの受け入れ条件と検索語、diff 外を含む全ヒットの `一致` / `食い違い: <finding ID>`、全ハンクの finding ID / `問題なし`、保証範囲は一覧に載った項目までであることを追加し、順 3 の grep の語の経路では第 4 列だけが `直した` / `該当しない: <理由>` である差分を明記する
...
- [ ] 3.1 `plugins/dev-workflow/scripts/` に、共通一覧契約と G が固定 SHA で再実行した grep のヒット集合を比較し、一致なら exit 0、差分があれば exit 1 と missing/extra の `file:line` を出すスクリプトを追加して 1.3・1.4 を Green にする。このスクリプトを一周目照合と順 3 の grep の語の経路の第 1 段で共用し、同経路固有の修正後 HEAD に対する本文・件数・削除行の第 2 段は既存どおり別に維持する。場合分けの軸の経路は同スクリプトの grep コマンド・4 列検証に通さず、既存どおり軸の全域との一致だけで閉じる
