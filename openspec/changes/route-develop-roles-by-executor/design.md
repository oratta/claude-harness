...
Thread lifetime follows the profile role rather than every transport call. A new profile role, an independent review, or any Codex delegation starts a fresh thread and receives only artifacts plus the required summary. Before every same-role Claude resume, the coordinator recomputes the currently effective budget cap. It uses SendMessage only when the thread's already-applied model is within that cap, retaining both the requested tuple and applied model. If the applied model exceeds the current cap, the coordinator does not resume it: after satisfying the existing phase-completion or stop-confirmation condition, it hands the work to a fresh same-role thread whose model is within the cap. The requested tuple remains unchanged, while requested value, new applied value, and adjustment reason are recorded. Thus a thread originally applied as `fable` moves to a fresh `opus` thread after `FABLE_BUDGET_MODE` changes to `exhausted`, or to a fresh `sonnet` thread after `SHARED_BUDGET_MODE` changes to `depleted`, instead of bypassing the current cap.
...
- [A profile role can be resumed after a return] → Reuse the named Claude thread only for the same profile role and only while its applied model remains within the current budget cap; otherwise satisfy the existing completion or stop-confirmation condition and hand off to a fresh capped thread, carrying artifacts and a concise summary instead of full conversation history and recording requested value, applied value, and adjustment reason.
