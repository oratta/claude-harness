...
- [版を上げる古い PR が後からマージされて `version` が復活し、更新が止まる] → 置き換え後の S130・S131 が CI で常に走り、main を取り込んでいない PR も取り込んだ PR も `version` があれば落ちる。main を取り込んでいない PR も落ちるのは、`pull_request` イベントの CI が `refs/pull/N/merge`（main との仮マージ結果）を checkout するので、main 側の新しい S130・S131 が走るためである。版を上げる PR は main が消した `version` の行を書き換えているので必ず衝突し、解消のときに `version` を残せば新しい検査で落ちる。周知（受け入れ条件 8）はマージ後に本体が行う
- [移行のマージ直後、利用者のキャッシュは semver 名から SHA 名に 1 回切り替わる] → 文字列一致の判定なので更新として扱われる（裏取り済み）。受け入れ条件 7 でマージ後に確認する
- [どのプラグインにも触れないマージでも、全 13 プラグインの版（commit SHA）が変わり、利用者のキャッシュに SHA 名のディレクトリがマージごとに増える] → 古いディレクトリの掃除は Claude Code の `.orphaned_at` による処理に任せる。動作は壊れない。版が変わっても中身が同じなので、再注入が 1 回余分に起きるだけである
