...
- **走査は `subagents/agent-*.jsonl` の 1 経路。隔離の有無は隣の meta.json で分類する**: `isolation: "worktree"` のサブエージェントも通常と同じ `subagents/` に置かれ、違うのはファイル名だけ（隔離すると名前が載らず `agent-<agentId>.jsonl` になる）。したがって受け入れ条件の「worktree 隔離のエージェントも集計対象に含まれる」は既存の glob で満たされ、追加の走査経路は要らない。隔離の有無は `agent-<id>.meta.json` の `spawnedWithWorktree` で判定し、経路別の統計を出す。
...
- **テスト**: `plugins/dev-workflow/tests/subagent-context-audit.bats`（新規。集計・期間切り・部分破損・fail-open・隔離エージェントの取りこぼし防止・サブエージェント以外の除外・キャッシュ TTL）。`scripts/test.sh` が `git ls-files '*.bats'` で自動発見する。
...
- **外部依存**: `~/.claude/projects` のディレクトリ構造とトランスクリプトの `usage` フィールドに依存する（`subagent-context.sh` と同じ依存で、Claude Code 側の変更で壊れうる。壊れたときは fail-open で空の結果になる）。加えて隔離の分類だけが `agent-<id>.meta.json` の `spawnedWithWorktree` に依存する（欠けても全体の件数・中央値は正しいまま）。
