...
- 子へ渡す環境変数の扱いを変える。`DROPPED_ENV`（親から落とす一律の 13 個）から `TMPDIR` と `TMPPREFIX` を外し、**全 role の子**に親の値がそのまま届くようにする。読む役（review / spec-review / impl-review / decider）だけ落とす扱いは作らない。現行コードに「読む役だから落とす」という分岐は無く、読む役に届かないのは worker が専用領域を作って `DROPPED_ENV` で一律に落としていたことの副産物にすぎない。これを残すには役割別の分岐を 1 つ増やすことになり、「Codex 側にだけある制約で Claude 側に対応物が無いものは外す」という今回の方針と逆向きになる（Claude 側の読む役は汎用サブエージェントで親の `TMPDIR` を引き継ぐ）
...
- `codex-worker`: 一時領域の要件を改める。（a）専用領域の新規作成・`TMPDIR` / `TMPPREFIX` の差し替え・別 job への再利用禁止・`job-tmp.json` への記録・確認済み終了後の削除を求める 2 つの Requirement を撤去し、書く役が親の `TMPDIR` を引き継ぐ要件に置き換える（b）read-only role に `TMPDIR` / `TMPPREFIX` を渡さないという役割別の扱いを取り除き、全 role が親の値のまま受け取ると定める（c）撤去によって失われる性質（job 間で一時ファイルが混ざりうる・残骸を追跡しない）を明記し、追跡が要るときは Claude 側と揃えて入れると定める（d）実 Codex の受け入れ証跡から専用領域の 0700 と終了時削除を外し、親 `TMPDIR` の一致と素の `mktemp -d` を使う外部スクリプトの完走に置き換える（e）書く役の Requirement に残っている「専用一時領域の作成・差し替え・削除は維持しなければならない」を取り除く

## Impact

- `plugins/dev-workflow/scripts/codex-worker.py`: `JobTmp` クラスの削除、`DROPPED_ENV` から `TMPDIR` / `TMPPREFIX` を外す、`Rpc.__init__` の `job_tmp` 引数の削除（読む役かどうかの引数は新設しない）、`worker()` の `job_tmp` 生成 / `job-tmp.json` 書き出し / `finally` の cleanup 分岐の削除、`turn_submitted` と `turn_accepted` の違いを示すコメント
- `plugins/dev-workflow/tests/test_codex_worker.py`: `JobTmpTest` クラス全体（6 件）と、`test_job_tmp_is_private_external_and_not_reused` / `test_tmp_cleanup_for_confirmed_outcomes_and_symlink_contents` / `test_unknown_retains_tmp_and_ownership` / `test_replaced_tmp_reports_cleanup_failure_without_following_symlink` / `test_invalid_tmp_parent_rejects_without_fallback` の削除。`test_read_only_child_gets_neither_tmpdir_nor_tmpprefix` の削除（読む役の子にも親の値が届く形に変わるため）。`test_child_inherits_parent_environment_except_the_dropped_names` と `test_all_role_policies_remain_restricted` の期待値の更新（後者は書く役・読む役とも「親の `TMPDIR` / `TMPPREFIX` と一致する」を見る形に直し、専用領域が残っている前提のコメントを外す）。書く役の子の `TMPDIR` / `TMPPREFIX` が親と一致することを固定する新規テスト
