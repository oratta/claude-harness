...
### 30 秒の予算は「検知 1 秒 + 中断の締切 10 秒 + 後始末 9 秒 = 20 秒」で組む。締切の起点は interrupt を送った時刻

受け入れ条件は合図から 30 秒以内にコマンドと app-server の両方が消えることを求める。台帳経路は `turn/interrupt` の**応答が返ったあと**に猶予を数え始める（`codex-worker.py:527`〜533）。この形をそのまま前景経路へ写すと予算が合わない。応答が 14 秒で返る場合、検知 1 秒 + 応答待ち 14 秒 + 猶予 10 秒 + 後始末 9 秒 = 34 秒になり、30 秒を超える。

そこで前景経路は締切を 1 つにまとめる。`turn/interrupt` を**送った時刻**を起点に 10 秒の締切を立て、interrupt 自身の応答待ちも、そのあとの `turn/completed` の待ちも、同じ締切までの残り時間を上限にする（`Rpc.request` が応答を待つ既定の 15 秒（`codex-worker.py:399`）を、残り時間で切り詰めた値で置き換える）。残りが 0 になったら応答を待たずに後始末へ進む。これで最悪ケースは「検知 1 秒 + 締切 10 秒 + `Rpc.close()` の段階的な後始末 9 秒（stdin クローズ 3 秒 → `terminate()` 3 秒 → `kill()` 3 秒）」の 20 秒に固定され、app-server が応答に何秒かけても 30 秒を超えない。台帳経路の猶予 20 秒とその起点（応答後）は変えない（回収する人がいる前提の経路なので、確認を待つ価値がある）。
...
account 名から CODEX_HOME への対応表は台帳の `accounts` テーブルにしかないので、前景経路では引けない。代わりに対応表を**呼び出し側の設定**として持つ。`codex-develop.py` が依頼ファイルを組み立てるときに、profile の役割から出てきた account 名をその設定で CODEX_HOME のパスに解決し、解決したパスを依頼に載せる。設定の形は `--account-home NAME=PATH`（繰り返し可）か、同じ対応を書いた JSON ファイル（`--account-home-file PATH`）。**JSON ファイルの形は account 名をキー・CODEX_HOME の絶対パスを値とする平らなオブジェクト 1 つ**とし、version キーや入れ子は持たない（`{"personal": "/Users/x/.codex", "spare": "/Users/x/.codex-spare"}`）。値が絶対パスでない、またはディレクトリとして存在しない項目があれば、その時点で拒否する。**`--account-home` と JSON ファイルの併用は拒否する**（既存の「profile と account/model の併用を拒否」に揃える）。片方が空でも両方指定されていれば拒否し、2 つを合成したり片方を優先したりしない。対応に無い account 名は拒否し、既定の home や別の home に置き換えない。エピック #345 の「実行先とアカウントの違いは『どこへ投げるか』の設定にだけ現れる」に沿う形で、台帳を読まずに実現できる。
...
**この決定で失われるもの**: 登録済み account だけに実行先を縛る性質を、台帳ではなく呼び出し側の対応表で保つことになる。あわせて、役割ごとの設定は run への snapshot ではなく委譲ごとの解決になるので、案件の途中で profile を編集すると後続の委譲に効く（台帳経路では snapshot 固定で守られていた性質。Claude 側のサブエージェント定義も spawn ごとに読まれるので、揃った振る舞いになる）。対応表を正しく与えるのが呼び出し側の責任で、worker 単体では「名前は `spare` なのに home は personal」という食い違いを検知できない。代案の「台帳を読み取り専用で開いて対応表だけ引く」は、台帳ファイルの存在と `register` 済みであることを前景経路の前提として残してしまうので採らない。
...
台帳を作らない完走は、`run` を実行したあとに state-dir に当たるディレクトリが無いこと（`ledger.sqlite` / `ownership.sqlite` が存在しないこと）と、stdout の JSON の項目を見る。親の消失は、本体が Bash ツールで背景実行する経路をそのまま写して 3 段で組む: テストが中間プロセスを起こし、その中間プロセスが `sh -c 'python3 codex-worker.py run …'` を起こし、その shell が `run` を起こす。`run` の直接の親は shell なので、kill するのは**中間プロセス**（`run` から見れば祖父）にし、`run` と app-server の両方の PID について `os.kill(pid, 0)` が失敗するまでを測る（テスト側の上限は 30 秒、想定は 20 秒以内）。SIGTERM は `run` に直接送り、偽 app-server が `turn/interrupt` を受け取った記録が残ることと、app-server の子プロセスが残らないことを見る。
...
- [祖先の連鎖の監視が 1 秒間隔なので、最悪 1 秒の検知遅れが出る] → 予算に組み込み済み（interrupt を送った時刻を起点にした 1 つの締切で組むので、応答が遅くても合計 20 秒で 30 秒の条件に収まる）
