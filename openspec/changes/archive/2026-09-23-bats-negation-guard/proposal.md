...
- 対象の定義（grep 1本で機械照合できる形）と、grep が通すことを許す対象外（`if`/`while`/`elif`/`until` の条件式・パイプライン途中の否定・行頭が `||` 等で始まる行・既にガード済みの行）を spec に明記し、ガード漏れを機械的に検査する常設テストを追加する。複数行にまたがる `! cmd` やヒアドキュメント本体内の `! ` 行はこの grep に誤って拾われるため、対象外ではなく「そう書かない」規約として扱う。
...
- ガードを付けた結果、現在 green だが実際は壊れていたアサーションが露呈して落ちるテストが出た場合は、この PR 内で実装側の欠陥を直す。`bash scripts/test.sh` の全件 green を本 change の受け入れ条件に残す（エピック #402 の目的が `scripts/test.sh` の green を信用できる状態にすることであり、ガードで露呈した失敗を先送りすると目的と矛盾するため）。
