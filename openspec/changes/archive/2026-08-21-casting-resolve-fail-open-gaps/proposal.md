...
- **HTML コメントの開閉不一致を検出する**: `<!--` と `-->` の個数不一致を `unclosed-comment` として findings に積む（配役表2層は resolve でも fail-closed、precedents.md は check モードで報告）
...
- `plugins/casting/scripts/casting-check.sh` — 引数パーサの書き直し、`check_comment_balance` 追加、列数判定を `-ne 6` に、resolve の配役表ゼロ分岐と対象不在チェックを追加
