...
- [x] 5.2 workspace-write role（implement / spec-write）を砂場なしにした。`thread/start` の `sandbox` を `danger-full-access`、turn の `sandboxPolicy` を `{'type':'dangerFullAccess'}` にし、`writableRoots` / `excludeSlashTmp` / `excludeTmpdirEnvVar` / `networkAccess` は砂場なしの policy が持たないので渡さない。read-only role（review / spec-review / impl-review / decider）は `readOnly` と role 別の `networkAccess` を維持した。**理由**: 5.1 で記録したとおり第 1 段では `git switch -c`（`HEAD.lock`）と `git commit`（`index.lock`）が `Operation not permitted` で拒否され（exit 128）、`WorkspaceWriteSandboxPolicy` にこれを解く項目が無い。Claude のサブエージェントには対応する隔離が無いので、対応物の無い制約を落とした。fake テストは 54 件全件成功・exit 0
- [x] 5.3 砂場なしの implement role で再実測し、issue #334 に記録した。**結果はすべて exit 0**: 環境変数の引き継ぎ（`HARNESS_PROBE_TOKEN` / `HARNESS_PROBE_SECRET_KEY` / `HARNESS_PROBE_PLAIN` が SET、`GH_TOKEN` は親に無いが `gh` はキーチェーンで動く）・ループバックへの HTTP（`http_code=200`）・`git switch -c codex-probe-20260920-2`・`git commit --allow-empty`（`c2640ac`）・`git push -u origin`・`gh pr create --draft`（PR #339）→ `gh pr close`・`gh issue comment 334`・`git switch oratta/codex-worker-claude`。専用領域は `drwx------`（0700）で job 終了後に不在。砂場が無いので `/tmp` と呼び出し元 `TMPDIR` への書き込みも通る（第 1 段では拒否されていた）。flatmate の `scripts/test-task-store-worker.sh` は別 job（cwd `/Users/oratta/orca/workspaces/flatmate/boot-trust-784`、HEAD `b8661a7`）で完走し **PASS=261 FAIL=0 / exit 0**、実行前後とも作業ツリーの差分は 0 件
- [x] 5.4 第 2 段で完了できなかった操作は**無い**。揃えられなかった項目が無いことを `plugins/dev-workflow/scripts/CODEX-WORKER.md` に明記した（あわせて実測は implement role で行い、spec-write は policy が同一なので同じ結果が当てはまることも書いた）

## 6. ドキュメントの整合

- [x] 6.1 `plugins/dev-workflow/scripts/CODEX-WORKER.md` を実態に直した。role ごとの段（書く役は砂場なし、読む役は `readOnly` ＋ role 別 `networkAccess`）・砂場なしの policy には `writableRoots` / `excludeSlashTmp` / `excludeTmpdirEnvVar` / `networkAccess` が無いこと・落とす環境変数 13 個と `GH_TOKEN` は渡ること・`shell_environment_policy` は絞らないので runtime 設定を足さないこと・専用一時領域に OS の強制が伴わなくなったこと・実測は implement role で行い spec-write は policy 同一なので同じ結果が当てはまること・揃えられなかった項目は無いこと
- [x] 6.2 `plugins/dev-workflow/references/codex-develop.md` の「共通workerはnetwork無効」と、GitHub 情報の取得・コメント / ラベル・Draft PR 作成・push・commit を `needs-coordinator` で本体に回す記述を削った（4 つとも worker の中で通り、commit も通った）。本体が代理するのは read-only role のレビュー結果と記録済みの項目だけである、と書き換えた。「これは運搬/記録の代理であり、仕様・コードの編集やレビュー判定を本体が代行するものではない」「Gへは操作結果の証拠を渡して確認させる」は残した。**`:46` の「read-only reviewerはGitHubコメントを書かない。本体が既存正本の書式で結果を代理投稿する」は残した**。あわせて hook の段の「workerのread-only sandbox以外のhook保証」を、書く役が砂場なしで動く実態に合わせた
- [x] 6.3 `plugins/dev-workflow/docs/codex-develop.md` の「workerはnetworkを使わないため…」の段落を同じ方針で直した。**`:37` の「GitHub コメント取得・保存は coordinator が代理する」は触っていない**（継続記録の話で、delta に MUST として残っている）
...
- [x] 6.5 確認した。delta（`openspec/changes/align-codex-worker-permissions/specs/codex-develop-continuation/spec.md`）は `## MODIFIED Requirements` に「coordinator と担当者の責務境界を守る」1 件だけを持ち、archive でその Requirement が delta の全文に置き換わる。置き換え後は「GitHub 操作と commit/push は workspace-write role の担当者が worker の中で自分で完了する」（本文と Scenario「書く役が GitHub 操作と commit/push を自分で行う」）・「代理は揃えられなかった項目として記録済みの操作だけ」（capability `codex-worker` の該当要件を名指し）・「read-only role のレビュー結果は coordinator が代理投稿する」（Scenario「読む役のレビュー結果を投稿する」）の 3 点を保つ。他の 4 Requirement（継続依頼は初回の Codex 設定を復元する・復元不能時は安全に停止する・継続復元は既存 develop の品質工程を変えない・継続復元の回帰を検証する）は delta に現れないので手が入らない。元の Requirement「coordinator と担当者の責務境界を守る」が、archive で delta（`openspec/changes/align-codex-worker-permissions/specs/codex-develop-continuation/spec.md`）の全文に置き換わることを確認する。置き換え後の本文が「GitHub 操作と commit/push は workspace-write role の担当者が worker の中で自分で完了する」「代理は揃えられなかった項目として記録済みの操作だけ」「read-only role のレビュー結果は coordinator が代理投稿する」の 3 点を保っており、他の 4 つの Requirement（継続記録の復元・安全停止・品質工程・回帰検証）に手が入っていないことを見る
