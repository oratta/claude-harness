...
- **判定条件を文・段落単位にも掛ける。** 常時注入から外してよいのは、読まれなくても挙動が変わらない部分（経緯・事故の説明、理由の説明、他の常時注入文書との重複、description の手順説明）に限る。外した文ごとの分類は PR 本文の移設表に載せる。`rules/README.md` の一覧表はルールごとの移設先までとし、文ごとの分類は持たせない（README を文単位の表にすると、ルールを直すたびに README の保守が重くなる。PR 本文はレビューの時点で文単位の判定を確かめるための記録として足りる）
