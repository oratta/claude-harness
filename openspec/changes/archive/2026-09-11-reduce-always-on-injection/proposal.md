...
- **path スコープ（rules frontmatter の `paths:`）の採否を実機確認で決める。** 公式仕様では一致するファイルを読んだときだけ載るが、ユーザーレベル `~/.claude/rules/` で効かない不具合報告がある（anthropics/claude-code#22170、#17204）。採用可否を先に 1 件で確かめ、効かなければ縮約と移設だけで目標に届かせる（削減率の目標は 2026-09-11 の見直しで撤回。design の決定 6）
