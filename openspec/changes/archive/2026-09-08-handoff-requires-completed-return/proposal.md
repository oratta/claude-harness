...
- **テストをホワイトリスト型に作り替える**: 「禁じたい言い回しを列挙して探す」形をやめ、「正本以外の面が手渡しに言及するなら、その箇所は正本への参照を含む」ことを検査する形にする。検査する面は `git ls-files` からの機械的な列挙とし、除外は歴史記録（`CHANGELOG.md` の過去項・過去 change の archive）と change の `proposal.md` / `tasks.md` だけにする。
