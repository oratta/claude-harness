...
`Bash` を持たないので `gh` でコメントを書けず、記録先の本文も自分では読めない。決める役の出力は呼び出し側（develop の本体・R1 を起こした本体・G）が受け取り、記録先への記録と次の実行役の spawn はそちら側で行う。決める役の責務は「原因の分類・具体的な指示・次の実行役のモデル」を返すことに限る。

これは既存の「記録先のコメントは各役割が自分で投稿する」（`SKILL.md` 47 行目）と衝突するので、R1 を `dev-workflow:decider` で起こす経路だけ例外にする: **判定結果は本体が R1 の return を同じ 1 行目書式（`^仕様レビュー: (APPROVE|REQUEST_CHANGES)$`）で代理投稿し、記録先の本文と関連コメントは本体が入力文に貼って渡す**。書式と「APPROVE が記録されるまで実装に進まない」判定は変えないので、pr-review-gate の機械照合はそのまま効く。R1 を従来どおり `general-purpose` + `model` で起こす経路では、R1 が自分で投稿する既存の手順を残す。

### ガードは残量モードを見ない（層を分ける）

ガードが判定するのは「誰が Fable になりうるか」という構造上の上限で、「今 Fable を使ってよいか」という残量の助言は develop 側の `FABLE_BUDGET_MODE` / `SHARED_BUDGET_MODE` が担う。ガードが usage snapshot を読むと、deny という強い判定が snapshot の鮮度と fail-open に依存してしまう（snapshot が古い・読めないときに判定が揺れる）。結果として `exhausted` の環境でも `dev-workflow:decider` への `model: fable` はガードを通るが、その状態で decider を Fable で立てないのは develop 側の役割選択の責務であり、種別は変えずに `model: opus` に切り替える。
...
### 仕様作成（`/opsx:ff`）は W の仕事なので opus 上限になる

W は実行役なので、この change のあとは仕様作成そのものが `opus` 止まりになる。決める役は Write を持たない設計なので、決める役に artifact を書かせる選択肢は無く、これは「Fable は決めるだけで手を動かさない」の一貫した帰結として受け入れる。判断の濃さは仕様化判断（W）・R1 レビュー（必要なら `dev-workflow:decider`）・本体（Fable）の 3 点で吸収する。

受け入れるコストは、**判断の濃い change で W（opus）の 1 周目が外れたときに、R1 の 2 周キャップから `needs-approval` で人に返る回数が増えること**。Fable が最初から artifact を書けば 1 周で決まったかもしれない案件が、opus + R1 の往復に置き換わる。頻度が問題になるなら、決める役に方針を出させてから W に書かせる（往復は増えるがモデル単価は上がらない）形で調整する。

逃げ道として「`/develop` に入る前に Fable の対話セッションで change を作っておく」形は禁じない。ただしその場合、W の (1) が既にある change を上書きしないことが前提になるため、`worker.md` の「仕様化する場合」の節に「対象の change が既に存在し artifact が揃っているなら `/opsx:ff` を再実行せず、そのまま本体に return する」を明記する（現行 92 行目は常に `/opsx:ff` を回す前提の書き方になっている）。
