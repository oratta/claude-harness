...
そこで「env から導出したサービス名で一致するスロット → 無ければ snapshot の `active`（probe が env から書いた値）→ それも無ければ最初のスロット」の三段にする。env が未設定・空文字のときも空文字からの導出（＝既定サービス名）として一段目を適用するので、既定スロットがあればそこで一致する。二段目に落ちるのは、既定スロットを持たないレジストリで env が未設定のときと、env がどのスロットにも一致しないときである。A と B のセッションを同時に開いても、probe 側の env は伝播が確認済みなので `active` は正しく書かれる。
