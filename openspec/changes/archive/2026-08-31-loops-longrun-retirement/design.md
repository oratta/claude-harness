...
- 新設 `plugins/dev-workflow/tests/retirement.bats`: 3 ディレクトリ不在・marketplace に 3 名が無い（`plugins[]` と bundles）・38 spec ディレクトリ不在・`grep -rn "loops:\|/lr:\|longrun" plugins rules docs README.md .claude-plugin .github scripts` の許容リスト外ヒット 0 件・CHANGELOG に uninstall 3 行
