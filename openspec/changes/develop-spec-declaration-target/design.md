...
- **bats は肯定＋否定の 2 面でアサートする**: 「入口 0 節内に `仕様宣言` を含む行がある」（肯定）と「入口 0 節内で `仕様宣言` を含む行はすべて `PR コメント` を含む」（否定: 節を切り出して `grep 仕様宣言 | grep -v 'PR コメント'` が空）。肯定だけだと従来文（記録先に置く）でも `PR` の語が節内に紛れて偽合格しうるため、否定側で従来文と入口 0 冒頭文（「仕様化判断・仕様レビュー結果・仕様宣言を置く「記録先」」）の再発を確実に落とす。R1 の SHOULD_FIX で「`記録先のコメントに置く` 行に仕様宣言が無い」から置き換えた（SKILL.md の文言への結合が弱い）。SKILL.md 側は記録先に置くものと PR コメントに置くものを別の箇条書き行に分け、行単位の grep で判定できる形にする
