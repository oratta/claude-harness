...
- 最終コンテキスト: ファイル末尾から固定サイズ（既定 256 KiB）の窓を読み、後ろから最初に見つかる `usage` 付き assistant レコードを使う。見つからなければ窓を倍にして最大 4 MiB まで広げ、それでも無ければその 1 件を**最終側の集計から除く**（全体の `count` と初回側の集計には残す。`last_median` / `last_max` / `over_cap_pct` の母数だけが `count` より小さくなる）。
...
- **meta.json が読めないときの退避は 1 段だけ**: meta.json が無い・壊れている 1 件のために集計全体を止めることはしない。その場合は `non_isolated` 側に数える（全体の `count` からは落とさない）。**ファイル名のパターンによる推定は入れない**。初回の仕様は「ファイル名で分類し、それも無理なら `non_isolated`」の 3 段にしていたが、仕様レビューで判定規則が一意に決まっていないと指摘され、実機では meta.json が 2022 件すべてに対応していて欠落が 0 件だったことから、**使われない分岐のための判定規則を契約に残さない**方を採った（テストのためだけに存在する規則になるため）。将来 meta.json の欠落が実際に観測されたら、そのときに規則を定義して足す。
- **Workflow 経由のサブエージェントは母集団に含めない**: `subagents/workflows/<wf-id>/agent-*.jsonl`（ネイティブ Workflow が起こしたサブエージェント）は `subagents/` の内側にあるが、固定深さの glob には当たらない（実機で 2 件。フラットな 2022 件とは別）。件数が小さく集計値を動かさないので走査経路には足さず、spec に「含めない」と明示するにとどめる。dev-workflow は `references/workflow-execution.md` で Workflow 経路を持つため、この母数は将来増えうる。増えたときに経路を足す判断ができるよう、`docs/usage-audit.md` に「この母数は集計に入っていない」と書き残す。
