...
- [ ] 1.4b fixture: サブエージェントでないもの 3 件 —（a）`<proj>--claude-worktrees-agent-abc123/<uuid>.jsonl`（worktree 内から起動された入れ子の `claude` セッション）、（b）`<proj>/<uuid>.jsonl`（メインセッション）、（c）`<proj>/<uuid>/subagents/workflows/wf_x/agent-y.jsonl`（Workflow 経由。`subagents/` の内側だが母集団に含めない）。いずれも集計に入ってはいけない
...
- [ ] 1.6b 失敗するテストを書く: 1.4b の 3 件（入れ子の `claude` セッション・メインセッション・Workflow 経由）が `count` にも `sources` のどちらにも含まれない
- [ ] 1.6c 失敗するテストを書く: meta.json が無い / 壊れている件も `count` に残り、`sources.non_isolated` に数えられる（`isolated.count + non_isolated.count == count`）
...
- [ ] 2.4b 分類: 各ファイルの隣の `agent-<id>.meta.json` を読み、`spawnedWithWorktree` が `true` なら `isolated`、それ以外は `non_isolated`。meta.json が無い / 壊れているときも `non_isolated`（ファイル名からの推定はしない。2 段）。どの場合も全体の `count` からは落とさない
...
- [ ] 3.4 `plugins/dev-workflow/docs/usage-audit.md` を新規作成する（`docs/auto-merge-deployments.md` の書き方に合わせる）。内容は ① 実行コマンド（`--days` / `--cap` / `--refresh`）② 出力キーの意味 ③ 何を見たら固定分が増えたと判断するか（全体の `first_median` を主系列に見て、動いたら `sources` で母集団の構成変化と切り分ける）④ キャッシュファイルの場所、の 4 点。あわせて母数の注意（`last_median` / `over_cap_pct` は最終が見つかった件だけが母数）と、Workflow 経由のサブエージェント（`subagents/workflows/<wf-id>/`）が母集団に入っていないこと・将来この母数が増えうることを書く
