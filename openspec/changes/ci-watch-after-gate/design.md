...
実装者がログを読んで PR の差分と関係ない失敗（表明で落ちる不安定なテストなど、ログの末尾からは本体が 2 条件を判断できなかったもの）だと判断したら、直さず push もせずに本体へ返し、本体は `next --unrelated <チェック名>` を 1 回だけ使う。同じ HEAD でのやり直しは 1 回までなので、判断を誤っても失うのは CI 1 周分で止まる。
