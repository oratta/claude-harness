...
- [ ] 1.3 `plugins/dev-workflow/agents/decider.md` を新設する（frontmatter の書式は `plugins/casting/agents/casting-arbiter.md` に倣う）。本文には入出力契約に加えて、記録先への投稿は呼び出し側の責務であること・Opus で立てるときも種別は `dev-workflow:decider` のままで `model` だけ変わることを書く。6.1 の `agents` 宣言と合わせて 1.1 / 1.2 が緑になることを確認する
...
- [ ] 3.6 決める役は段階によらず `subagent_type: dev-workflow:decider` に固定して `model` だけ切り替える（`general-purpose` に読み替えない）ことを、3.2〜3.5 で触る各ファイルのラダー記述に含める
- [ ] 3.7 `rules/subagent-model-selection.md` に「最上位ティアは決める役の種別（`dev-workflow:decider`）でだけ spawn する」「強制層は `plugins/dev-workflow/scripts/agent-model-guard.sh`」を各 1 行足す
- [ ] 3.8 3.1 が緑になり、`tests/rules-sync.bats` も緑であることを確認する
...
- [ ] 4.5 R1 を `dev-workflow:decider` で起こす経路の記録手順を反映する: `plugins/dev-workflow/skills/develop/references/roles/spec-reviewer.md` の「結果を記録先に記録する（R1 が投稿する）」節（62 行目付近）と「レビュアーへの入力」節（32 行目付近）に、decider 経路では R1 が `gh` を実行せず return し本体が同じ書式で代理投稿すること・記録先の本文と関連コメントは呼び出し側が入力文に貼ることを書き分ける
- [ ] 4.6 `plugins/dev-workflow/skills/develop/SKILL.md` 47 行目「記録先のコメントは各役割が自分で投稿する」に decider 経路の例外（本体が代理投稿する）を足し、1 ループの (2) の行にも同じ例外を書く
- [ ] 4.7 `plugins/dev-workflow/skills/develop/references/roles/worker.md` の「仕様化する場合」（92 行目付近）に、対象の change が既に存在し artifact が揃っているなら `/opsx:ff` を再実行せずそのまま return する旨を足す
- [ ] 4.8 4.1 の 4 テストが緑になり、`plugins/dev-workflow/tests/spec-decision-and-review.bats` と `develop-skill.bats` が緑であることを確認する
...
- [ ] 6.1 `plugins/dev-workflow/.claude-plugin/plugin.json` に `agents: ["./agents/decider.md"]` を足す（casting プラグインの宣言に揃える）
- [ ] 6.2 `plugins/dev-workflow/.claude-plugin/plugin.json` を 2.4.1 → 2.5.0 に上げ、`.claude-plugin/marketplace.json` の該当バージョンも合わせ、両ファイルの `description` に残る旧表の文言（「マージ権限・層間契約・課金/法務は 1 周目から fable」「実装品質起因のときだけ前回モデルの 1 段上（sonnet → opus → fable）」）を新しいラダーと事前分類に書き換え、`CHANGELOG.md` に追記する
- [ ] 6.3 `scripts/test.sh` を全件実行し、exit code と失敗件数を出力に残す（緑であること）
- [ ] 6.4 `openspec validate fable-decider-only --strict` が通ることを確認する
