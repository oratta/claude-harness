...
2. **失敗ループ → 決める役 / 実行役の分離ラダー**: 同じテストが2連続で落ちた、または同じ箇所を2回書き直した。実行役を1段ずつ上げるのではなく、失敗の原因が「決めた内容が間違っていた（判断側）」か「決めたとおりに実行できなかった（実行側）」かで、上げる相手を**一方だけ**決める（両方同時に上げない）。決める役は数ターンで終わるので先に上げ、実行役を上げるのは実行側が原因と分かったときだけとする（SHALL）。ラダーは次のとおり:
   ...
   | 1 回目の失敗 | `opus`（本体が opus 以上ならその場で。sonnet なら spawn）が失敗の出力を読み、直す箇所・方法・確認するテストを具体的な指示にする | `sonnet` で再試行 |
   | 2 回目の失敗・指示どおりやって結果が違う | `opus` | `opus` |
   | 2 回目の失敗・指示を解釈できなかった / 指示自体が外れていた | `fable`（`dev-workflow:decider` として spawn。本体が fable ならその場で） | `sonnet` または `opus` |
   | fable が決めて opus が実行しても落ちる | 人間へ（interactive は AskUserQuestion、unmanned は `needs-approval`） | — |

   実行役の上限は `opus` であり、実行役を Fable で spawn してはならない（MUST NOT。強制層は `plugins/dev-workflow/scripts/agent-model-guard.sh`）。`FABLE_BUDGET_MODE=reserve` の自動実行、および `exhausted`（全経路）では決める役も Opus 上限とする。共有枠モード `SHARED_BUDGET_MODE` の下限が先に効く（`depleted` は昇格なし、`throttled` は Opus 上限）
...
- **THEN** 実行役は `sonnet` のまま再試行し、`opus` の決める役が失敗の出力を読んで直す箇所・方法・確認するテストを具体的な指示にする
...
- **THEN** interactive / unmanned を問わず昇格は Opus までに留まり、決める役も Fable へは昇格しない
