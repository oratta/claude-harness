...
- `codex-worker`: 砂場の要件を改める。（a）書く役は砂場なし（`danger-full-access`）で動き、worker の中で GitHub 操作と commit / push を自分で完了できる（b）読む役は書き込みを増やさずネットワークだけ揃える（c）子へ渡す環境変数の範囲と、渡してはいけない変数（d）書く役を砂場なしにした根拠（第 1 段の失敗出力と砂場の拒否であることの確認）を実測の証跡とともに記録する（e）実 Codex の受け入れ証跡に localhost・Git・GitHub・外部リポのテストスクリプトを加える
...
- `plugins/dev-workflow/scripts/codex-worker.py`: `clean_env()`、turn の `sandboxPolicy`（書く役は `dangerFullAccess`）、`runtime_home()` の config 生成
