...
実装は緩める段を順に試さなければならない（MUST）。第 1 段は workspace-write + networkAccess true + Git 共通ディレクトリの追加、第 2 段はその役だけを砂場なし（danger-full-access）にすることである。第 2 段へ進む前に、第 1 段で失敗したコマンド・出力・exit code を記録しなければならない（MUST）。実測の証跡なしに砂場なしを既定にしてはならない（MUST NOT）。段の判断は role ごとに独立に行わなければならず（MUST）、ある role の不足を理由に他の role を緩めてはならない（MUST NOT）。
...
#### Scenario: 第 1 段で足りない
- **WHEN** 第 1 段で完了できない操作があり、失敗したコマンド・出力・exit code が記録されている
- **THEN** その role だけを砂場なしにしてよく、緩めた role と理由が記録に残る

#### Scenario: 証跡がないまま緩める
- **WHEN** 第 1 段の失敗の記録が無い状態で砂場なしの設定を入れようとする
- **THEN** その変更は受け入れられない
