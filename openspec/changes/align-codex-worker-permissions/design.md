...
3. 実 Codex の implement role で受け入れ対象の操作を実測する。足りない role があれば、失敗の証跡と「その失敗が砂場の拒否によるものだと確認した根拠」の両方を記録してから第 2 段へ（原因が環境変数の不達・スクリプトの不具合・一時的な通信失敗なら第 2 段へ進まず、原因を直して第 1 段で再実測する）
