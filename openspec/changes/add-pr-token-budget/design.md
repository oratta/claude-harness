...
- executor が `codex` の役割（Codex CLI 経由の W・レビュー）と Workflow 経由のサブエージェントの計測。Codex の消費は Claude のトランスクリプトに残らないので、この上限の外になる。`codex-standard` のように W が Codex の profile では、実装の繰り返しに対する累計の歯止めがこの仕組みでは効かない。Codex 側を記録先単位で累計する仕組みは follow-up issue として切り出す（範囲外。起票は本体が行う）
...
本体がその記録先のためにサブエージェントを spawn する直前（W / R1 / G / G のレビュアー / 決める役はその例で、役割を問わない）、SendMessage で再開する直前、および executor が `codex` の役割へ委譲する直前。executor を問わず毎回測る（Codex への委譲の前に測っても合計は Claude 分だけだが、Claude 分が上限を超えていればそこで止まる。測る位置を executor で分けないことで、手順を 1 通りにする）。`subagent-context.sh` を再開前に呼ぶのと同じ位置に並べる（こちらは spawn と委譲の前にも呼ぶ）。最初の spawn の前はサブエージェントが 0 体なので合計 0 で exit 0 になる。

exit 1（引数エラー・python3 が無い・カレントディレクトリがリポジトリでない）のときは止まらずに進み、記録先に計測できなかった旨をコメントする（観測の失敗で作業を止めない。止めるのは上限超を確認できたときだけ）。コメントは同じ記録先・同じ理由について 1 サイクルに 1 回までにする（spawn のたびに同じコメントが増えるのを防ぐ）。
...
- [W が Codex の profile では累計が止まらない] → 範囲外として明記し、Codex 側の記録先単位の累計は follow-up issue に回す。Claude で起こした R1・G・決める役の合計は引き続き止まる
