...
- [ ] 1.3 spec の各 Scenario を bats のテストにする（合計・体数の手計算の期待値、`#2880` 不一致、二重計上なし、別リポジトリ除外、`unresolved`、`skipped_lines`、`--cap` 超過で exit 2、0 体で exit 0、不正な `--cap` で exit 1、`DEV_WORKFLOW_PR_TOKEN_CAP` の反映）
- [ ] 1.4 SKILL.md の手順を検査するテスト（`pr-token-budget.sh`・exit 2 で spawn / SendMessage しない・`続けるか、範囲外として閉じるか`・description の `#N` 規約・`PR トークン上限:` の文字列）を `tests/develop-skill.bats` に足す
...
- [ ] 3.1 `plugins/dev-workflow/skills/develop/SKILL.md` に、spawn / SendMessage の直前に `pr-token-budget.sh` を実行し exit 2 なら止まる手順、description に記録先番号 `#N` を入れる規約、「続ける」後の `PR トークン上限:` コメントと `--cap`、「範囲外として閉じる」後の扱い、exit 1 の扱いを書く（1 ループの (1)〜(4) の各 spawn / 再開の位置から参照できる形で 1 箇所にまとめる）
