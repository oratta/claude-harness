...
- `run` は**呼んだプロセスと一緒に終わる**。起動時に控えた親 PID を監視し、親がいなくなったら実行中のターンを中断して app-server の子プロセスごと終わる。合図から 30 秒以内に、コマンド自身と app-server の両方が消える
...
- `codex-develop.py` の役割ごとの設定解決（`resolve_execution`）と指示文の組み立て（`prompt`）を、前景実行の依頼ファイルを組み立てるところから使えるようにする
