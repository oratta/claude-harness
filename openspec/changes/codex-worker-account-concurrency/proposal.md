...
- アカウント単位の排他を「1 件」から「設定で決まる上限 N 件」に変える（既定 N = 3）。**BREAKING**: submit の拒否理由 `cwd_or_account_locked` が、作業ディレクトリ側の `cwd_locked` とアカウント側の `account_slots_exhausted` に分かれる。
...
- 残枠の判定に余裕の幅を持たせる。使用率が 100% 未満であることだけを見るのをやめ、同時に走っている本数 × 1 本あたりの見込み消費率を足しても 100% を超えないことを要求する（見込み消費率の既定は 5%）。
- 同時実行をサーバー側が拒否した場合は、理由を台帳に残して停止する。再試行も別アカウントへの振り替えもしない。
- 受領されないまま終わったジョブが握り続けているスロットを外すコマンド `reap` を追加する。
...
- 未アーカイブの change `add-codex-worker` の delta にアカウント側の排他を 1 件と読める MUST が残るので、どちらかがアーカイブされる時点で文面の突き合わせが要る。
