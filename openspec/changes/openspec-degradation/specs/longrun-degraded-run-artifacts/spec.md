# longrun-degraded-run-artifacts

縮退モードの run が OpenSpec CLI なしで spec 類（proposal / tasks / verification-guide 相当）を `_longruns/<run>/` 内に自己完結生成し、Archive まで完走する capability。
