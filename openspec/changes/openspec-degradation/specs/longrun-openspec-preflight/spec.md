# longrun-openspec-preflight

exec 起動直後（Step 0）に OpenSpec の前提条件（`npx openspec` 解決可能 + openspec init 済み）を検査し、満たさない場合は AskUserQuestion で縮退モードを提案して run の動作モード（通常 / 縮退）を確定する capability。
