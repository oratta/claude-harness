...
`plugins/infra/templates/workflows/*.yml.template` の `uses:` のうち GitHub 公式（`actions/*`）以外のものは、40 桁のコミット SHA で参照し、その SHA が指すバージョンを同じ行のコメントに併記しなければならない。テンプレートの展開先では production のデプロイ／マイグレーションという secrets を持つジョブで動くため、可変タグ／ブランチ参照（例: `supabase/setup-cli@v2` は tag ではなくブランチ）は上流の push だけで別コードの実行につながる。この検査は「抽出」「公式判定」「コメント形」の 3 点で fail-closed でなければならない — 抽出は行頭のキー形（`uses[[:space:]]*:`。コロン前の空白と行頭の `- ` を許容する）を対象とし、公式判定は行全体の部分一致ではなく `uses:` の値の位置で行い、コメントはバージョンらしい形（`v` 省略可の数値列。直後は空白か行末で終わる）でなければならない。引用したキー（`- "uses": ...`）と flow mapping（`- { uses: ... }`）は YAML としては有効だが本検査の適用外であり、この 2 形の抽出は oratta/claude-harness#182 で扱う。This requirement MUST be satisfied.

#### Scenario: Every third-party `uses:` in the templates carries a SHA and a version comment

- **WHEN** `plugins/infra/templates/workflows/` 配下の全 `*.yml.template` から、行頭のキー形（正規表現 `^[[:space:]]*-?[[:space:]]*uses[[:space:]]*:`）に一致する行を全数抽出し、その `uses:` の**値**（行内で最初に現れるキーの値。前後の引用符は除く）が `actions/` で始まらないものを検査する
- **THEN** 値が `<owner>/<action>@<40 桁 16 進数>` の形（`<owner>` に `/` を含まず、`<action>` は `repo/subdir` のようなサブパスを許す）でなければならない
- **AND** 同じ行の**YAML 本文**のうち、**行内で最初に現れる `#` から始まる部分**（＝コメント部分）の先頭が、バージョンらしい形（`#` の直後に空白を挟んでよく、`v` は省略可、`1` / `v2.1.1` のような数値列で、直後は空白か行末）でなければならない。コメント部分より前や途中に現れる `#<数字>`（例: `# TODO #176 follow-up`）を判定材料にしてはならない。走査ツールが前置するファイルパス・行番号も判定材料にしてはならない（前置の剥がし方は走査対象のパスが `:` を含んでも壊れない方式でなければならない）
