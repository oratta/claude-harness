...
#### Scenario: コードフェンス内の記入例は実在の上書き行にならない

- **WHEN** コードフェンスの中に記入例の表行（コメントで囲んだものと囲まないものの両方）を置き、その後ろに同じ観点の上書き行が続く配役表に対して resolve を実行する
- **THEN** exit code が 0 になり、resolve は記入例ではなくフェンスの後ろの上書き行を由来 `project` で出力する
