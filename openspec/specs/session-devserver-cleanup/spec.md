# session-devserver-cleanup Specification

## Purpose
Claude のセッションが Bash ツールから起動したプロセス（dev サーバー等）を、セッション終了時（落ちたセッションは次のセッション開始時の掃除）に止める。目印の環境変数を持たないプロセスや目印を外して起動したものは止めない（issue #470）。
## Requirements
