...
SKILL.md は、決める役（`dev-workflow:decider`）の裁定が既存の出力契約（原因分類・直し方・次のモデル）のままであり、周回キャップの判定（全周共通の判定による仕分け・3 周目に入るかどうか）に関与しないことを明記しなければならない（MUST）。仕分けと 3 周目に入るかどうかは、全周共通の判定で止める指摘が残るかどうかで決まる（MUST）。キャップで主に上げる際に決める役を挟んではならない（MUST NOT）。

#### Scenario: キャップ到達時に決める役を起こさない

- **WHEN** 2 周目終了時に全周共通の判定で止める指摘が残り、主に上げる
- **THEN** 決める役は起こされず、G が `needs-approval` を付けて主に 1 択を出す

### Requirement: G の指示書の failed 節と周回欄が収束ルールに揃っている

`skills/develop/references/roles/gate-runner.md` は、failed 節と return の書式を SKILL.md の収束ルールに揃えなければならない（MUST）: 2 周目の return では指摘ごとの仕分け（引用・例外 3 種のどれか、または follow-up issue の URL）を含めること、全周共通の判定で止める指摘が残った場合は failed ではなく保留（`needs-approval`）として return し 3 周目を提案しないこと、`周回:` 欄は主が続行を指示した場合の 3 周目以降も表せること、全体レビューにした周はその旨と判定に使った 2 つの行数を書くこと。あわせて再開節を揃えなければならない（MUST）: 「W の修正後の再レビュー」の記述は、再レビューの範囲に全体レビューの例外を含め、3 周目の条件を主の続行指示に置き換え（「3 周目に入れるのは新規の高深刻度 blocking のみ」の許可条件を残さない）、「保留の解除」は 2 周目キャップへの主の回答（続ける／範囲外として閉じる）を含めること。

#### Scenario: 2 周目で止める指摘が残ったときの return

- **WHEN** G が 2 周目を終え、全周共通の判定で止める指摘が残っている
- **THEN** return は Status: 保留で、引用した違反文（または例外 3 種のどれか）と指摘、主に依頼する 1 択（続けるか、範囲外として閉じるか）を含み、W の再開や 3 周目を提案しない
...
SKILL.md 手順 5 は、合格処理の前提として、最後のレビュー結果に全周共通の判定で止まる指摘（`blocking` かつ `confirmed` で、G が引用を照合済みまたは例外 3 種に当たるもの）のうち、主が範囲外として閉じて follow-up issue に切ったもの以外が 0 件であることを書かなければならない（MUST）。2 周目キャップで主が「範囲外として閉じる」と答え、follow-up issue に切った指摘は、この 0 件の数に含めない（MUST）。

#### Scenario: 手順 5 の合格条件

- **WHEN** SKILL.md 手順 5 を読む
- **THEN** 全周共通の判定で止まる指摘（`blocking` かつ `confirmed`、G が引用を照合済み）のうち、主が範囲外として閉じて follow-up issue に切ったもの以外が 0 件であることが合格処理の条件として書かれている
