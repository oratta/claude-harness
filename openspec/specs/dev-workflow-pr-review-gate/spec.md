...
- grep の語のときの表の列は `| ファイル | 行（修正前 SHA） | ヒットした行の本文 | 扱い |`。場合分けの軸のときも `修正前 SHA:` の行は同じく書き、`検索コマンド:` の行には軸とその全域（例: 先頭バイト 0x00〜0xFF）を書き、表の列は `| 軸の値 | 扱い |` とする
...
2. HEAD で同じ検索コマンドを実行し、残ったヒットがすべて、扱いが「該当しない」の行に対応するかを見る。対応は「ファイル」と「ヒットした行の本文」の組で取り、行番号では取らない。同じ組が複数あるときは件数で照合する。ただし、同じファイル・同じ本文の組に「直した」と「該当しない」の両方があるとき（扱いが混在する組）は、件数の一致だけでは直し忘れを見分けられないので、あわせて `git diff <修正前 SHA> HEAD -- <ファイル>` の出力のうち `-` で始まりその本文が続く行の件数が、その組の「直した」の件数以上であることを見る（MUST）。足りなければ、その組の「直した」の行を「直したはずが残った」差分として W に返す

場合分けの軸のときは、ヒットの集合がコミットによらず軸の全域で決まるので、G は 1 段目として表の行の集合が軸の全域と一致するかだけを見て、2 段目を行わない。どちらの段でも一致しなければ、その差分（表に無いヒット、ヒットに無い表の行、「該当しない」の行に対応しない HEAD のヒット、扱いが混在する組で削除行が足りない「直した」の行）だけを W に返す。差し戻し（表の出し直しを求めた場合を含む）は 1 回までとし、2 回目も一致しなければ順 6 の型に落とす（MUST）。検索コマンドや軸の全域を W が書けない指摘は、順 3 に当ててはならない（MUST NOT）。
