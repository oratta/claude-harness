# dev-workflow-pr-token-budget Specification

## Purpose
TBD - created by archiving change add-pr-token-budget. Update Purpose after archive.
## Requirements
...
#### Scenario: 引数が足りない
- **WHEN** `--repo` か番号を渡さずに実行する（`--out` の `<file>` には前回の記録が残っている）
- **THEN** exit 1 で、`<file>` は存在しない
