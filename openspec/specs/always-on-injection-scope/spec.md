...
#### Scenario: 常時注入の合計が削減後の予算に収まる

- **WHEN** `tests/injection-budget.txt` を削減後の実測に合わせて置き直した状態で `scripts/test.sh injection-budget` を実行する
- **THEN** exit code が 0 である（予算超過の側でも、予算過大の側でも落ちない）。削減率が特定の値に達していることは条件にしない
...
この判定は、ファイル単位だけでなく**縮約する各ルールの中の文・段落単位にも掛けなければならない**（MUST）。ルールのファイルを縮約してよいと判定した場合でも、その中の一文が「読むべき瞬間に、読もうと判断できない」指示であれば、その文は短い形で常時注入に残さなければならない（MUST）。常時注入から外してよいのは、**読まれなくても挙動が変わらない部分**に限る（MUST）。具体的には次の 4 種類をいう。

- 経緯・事故の説明
- 理由の説明（何を守るかは残し、なぜそうするかの説明だけを外す）
- 他の常時注入文書（`rules/*.md`・`CLAUDE.md`・`output-styles/*.md`）との重複
- `description` の手順説明

外した文ごとに、上の 4 種類のどれに当たるかを PR 本文の移設表に載せなければならない（MUST）。`rules/README.md` の一覧表はルールごとの移設先までを持ち、文ごとの分類は持たなくてよい。

次の指示はこの条件に当たるため、縮約したルールの中でも短い形で常時注入に残さなければならない（MUST）。
...
- **WHEN** 縮約した各ルールについて、縮約前の本文と縮約後の本文を突き合わせ、外した文を 1 つずつ PR 本文の移設表と照らす
- **THEN** 外した文がすべて移設表に載っており、それぞれ「経緯・事故の説明」「理由の説明」「他の常時注入文書との重複」「`description` の手順説明」のいずれかに分類されている。どれにも当たらない文が 1 つも無い
