# dev-workflow-memory-refresh Specification

## Purpose
リポジトリの外にあって注入予算のテストで測れないメモリ（`~/.claude/projects/<project>/memory/`）について、索引の肥大と放置を SessionStart で通知し（通知のみ・fail-open）、主の承認と控えを取ってから手で見直す手順（memory-refresh スキル）を定める。
## Requirements
