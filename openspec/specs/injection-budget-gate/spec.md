...
この書式検査の守備範囲は、このリポジトリの作者（主と Claude）が書く skill ファイルのうっかりミスを拾う lint であり、悪意ある入力や任意のバイト列に対する防御ではない。判定は YAML パーサではなく行指向の近似で書かれているため、YAML として不正でも検査を通る入力が残ることを許容する（MAY）。近似である以上、まだ見ていない書き方は原理的に尽きないので、穴が見つかるたびに条件を足して塞ぎ切ることを完了条件にはしない。実際に残っている例として、引用符を閉じたあとに続きがある次の 3 つはいずれも検査を通るが、YAML パーサでは構文エラーになる: `description: "a" "b"`、`description: "abc"def"`、`description: 'it's'`。この 3 つは「通ることを保証する」対象ではなく、守備範囲の外にあることを示す例である。不正な `description` を持つファイルは、その skill が読み込まれないことで別途検知される。
