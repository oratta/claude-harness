...
手渡し規則の本文（上限超過を検知したときに送ってよい SendMessage と送ってはならない SendMessage、手渡しを行ってよい条件、W / G の return の 1 行目の宣言、前任が動作中のまま交代させるときの手順）は、`plugins/dev-workflow/skills/develop/references/decision-criteria.md` の「コンテキスト上限（サブエージェントの手渡し）」節にだけ置かなければならない（MUST。以下この節を正本と呼ぶ）。正本は次の 4 点を規定しなければならない（SHALL）: ①上限超過（exit 2）を検知したときに送ってよい SendMessage と送ってはならない SendMessage、②手渡し（前回の return と記録先を渡して同じ役割の新しいエージェントを spawn すること）を行ってよい条件、③W / G の return の 1 行目の宣言書式と、どちらの宣言を選ぶかの義務（固定された 2 つの書式のどちらにも当てはまらない return をどう扱うかを含む）、④前任が動作中のまま交代させる必要があるときの手順と、その待ち方（停止の指示に応答が返らないまま時間が過ぎたときの終端を含み、その終端は interactive の具体的な閾値・unmanned の終端との関係・終端に達したあとの本体の次の行動の 3 つを備える）。**この spec は①〜④の答えを再掲してはならない（MUST NOT）**。同じ規則の言い換えが 10 前後の面に散らばっていたことが、2026-09 に 3 周続けて書き換え漏れを出した原因であり、言い回しの多様性そのものを構造的に無くすことがこの要件の目的である。

③については、扱いを規定していることだけでなく、その方向まで spec が固定する。正本は、本体が工程の完了を判定する材料を W / G の宣言そのものに限り、return の内容から完了を推し量ることを禁じなければならない（MUST）。2026-09-09 に同じ worktree へ実行役が 2 人並んだ事故の直接原因は、テスト完了待ちで一時的に応答が止まっていた前任の return を、本体が成果一覧という内容から工程の終わりだと判断したことだった。③を「扱いを規定していること」だけの索引に留めると、正本が逆向き（内容から判断してよい）に書き換わっても spec 準拠のままになる。書式リテラルの固定と同じく、これは規定すべき点の方向の固定であって①〜④の答えの再掲ではない（SHALL）。

④の終端の閾値は本体のターン数で数え、1 回では足りず 10 回を超えない範囲で正本が具体値を定めなければならない（SHALL）。具体値そのものは正本が持ち、この spec は範囲だけを固定する。妥当性の根拠は `plugins/dev-workflow/references/subagent-waiting.md` の前景ポーリング契約（本体は自分のターンの切れ目で SendMessage を受信する）に置く。範囲が無いと、正本が「無制限に待つ」や「1 回で諦める」に書き換わっても spec 準拠のままになる。
...
- **③宣言の書式と選び方**
  ...
  - 述語（文末）: `完全一致` / `に一致` / `にする` / `にすること` / `にせよ` / `にしなければ` / `としなければ` / `でなければ` / `を選ぶ` / `どちらか` / `宣言する` / `宣言してはならない` / `宣言義務` / `推し量` / `読み替え`
...
前段で名指しした参照面と正本が、現に検査対象（機械的な列挙から除外を引いた集合）に入っていることを検査しなければならない（MUST）。除外を広げれば任意の面を検査から外せて全テストが緑のまま通るので、除外が上の 3 種を超えて広がっていないことの下限をここで固定する（実測: 名指しした 7 つの面と 2 つの live spec を除外に足しても全テストが緑だった）。検査対象に必ず含まれなければならない面は次の 10 個とする（MUST）: `plugins/dev-workflow/README.md`、`plugins/dev-workflow/skills/develop/SKILL.md`、`plugins/dev-workflow/skills/develop/references/roles/worker.md`、`plugins/dev-workflow/skills/develop/references/roles/gate-runner.md`、`plugins/dev-workflow/templates/escalation-tripwires.md`、`plugins/dev-workflow/scripts/session-tripwires.sh`、`plugins/dev-workflow/scripts/subagent-context.sh`、`openspec/specs/dev-workflow-execution-strategy/spec.md`、`openspec/specs/dev-workflow-develop/spec.md`、および正本 `plugins/dev-workflow/skills/develop/references/decision-criteria.md`。検査の実装は、機械的な列挙から除外を引いた検査対象の集合そのものを外から列挙できる手段（サブコマンド等）を備えなければならず（MUST）、テストはその出力にこの 10 個がすべて現れることを検査しなければならない（MUST）。実装が実際に見ている集合を外から取り出せないと、この下限の検査はテスト側で集合を組み直すことになり、実装の除外が広がってもテストは緑のまま通る。

参照面に書かれた正本へのパスは、実在するファイルに解決しなければならない（MUST）。解決の基準はリポジトリルート・その面自身のディレクトリ・プラグインルートの 3 つとし、`${CLAUDE_PLUGIN_ROOT}` はプラグインルートに置き換えてから見る。参照とみなす判定は文字列を含むかしか見ないため、パスが壊れていても参照として合格する。参照だけになった面はポインタが壊れるとその面で機構が失効し、「規則を知らないまま即興する」状態に戻る。
...
#### Scenario: 完了の判定材料を宣言だけに限ることを正本が定める
- **WHEN** 正本の「コンテキスト上限（サブエージェントの手渡し）」節が、本体が return の内容から完了を推し量ることを禁じなくなった状態でテストを実行する
- **THEN** テストは落ちる（③は扱いを規定していることだけでなく、方向まで固定されているため）
...
#### Scenario: 応答が返らないまま時間が過ぎたときの終端も正本が決める
- **WHEN** 本体が前任へ停止を指示したのに、前任がツール実行中で読まないまま時間が過ぎる
- **THEN** interactive と unmanned それぞれの終端は正本が規定しており、この spec は答えを再掲しない
- **AND** interactive の終端は本体のターン数の具体値で示され、その値は 1 より大きく 10 以下である（この spec は範囲だけを固定し、値そのものは正本が決める）
- **AND** 正本には終端に達したあとの本体の次の行動が書かれている

#### Scenario: 名指しした面は検査対象から外せない
- **WHEN** 除外の条件を広げて、名指しした参照面や正本を検査対象から落とす
- **THEN** テストは落ちる（除外が 3 種を超えて広がっていないことの下限を固定しているため）
- **AND** 検査対象の集合そのものを実装から列挙し、名指しした 9 つの参照面と正本の 10 個がその出力にすべて現れることを確かめている（テスト側で集合を組み直さない）

#### Scenario: 正本へのパスは実在するファイルに解決する
- **WHEN** 参照面が正本を指すパスを書く
- **THEN** そのパスはリポジトリルート・その面自身のディレクトリ・プラグインルートのいずれかから実在するファイルに解決する（`${CLAUDE_PLUGIN_ROOT}` はプラグインルートに置き換えて見る）
