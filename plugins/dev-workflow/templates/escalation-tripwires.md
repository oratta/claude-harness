...
4. 【コンテキスト上限 → 手渡し】
   ...
   → 再開しない（無条件）。**手渡してよいのは前任の直近 return の 1 行目が `工程完了: <工程名>` のときだけ**
      （`工程中断: <理由>` なら、バックグラウンドコマンド待ち等で工程が終わっていないので手渡さない。
      宣言契約の正本は `references/decision-criteria.md`「コンテキスト上限」）。手渡すときは前回の
      return（編集済みファイル・通ったテスト・判明した事実・埋めた決定・残作業）と記録先を渡して
      新しいサブエージェントを spawn する（モデルは変えない）。
