...
- 動作確認の**証拠**（HEAD SHA 付き）と**実装とドキュメント文字列の整合確認**の証拠を添付し、リスク宣言と合わせた3見出しが現在の HEAD SHA で実在することを API で実測してから `agent-review:passed` を付ける
- 収束ルール: レビューは既定2周キャップ・再レビューは差分限定・マージ後に直せるものは blocking にしない（**例外: 実装とドキュメント文字列の食い違いは同じ PR の中で直す**。合格＝即マージでマージ後に直す機会が無いため）
