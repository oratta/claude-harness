...
develop スキルの 1 ループ（W → R1 → W → G）で収まらない規模——昇格トリップワイヤー 1（規模超過）が発火し、エピック化（子 issue への分割）でも足りない探索的な実装——を、**ネイティブの Workflow ツール**で回すときの型。
...
- これは Workflow への乗り換えの話であって、コンテキスト上限に達した W / G の手渡しとは別の規則である。手渡しの条件・宣言の書式・前任が動作中のまま交代させる手順は `skills/develop/references/decision-criteria.md`「コンテキスト上限（サブエージェントの手渡し）」が正本で、ここには書かない
