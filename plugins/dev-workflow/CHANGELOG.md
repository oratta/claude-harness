...
## 2.5.1 — 2026-09-09: 手渡し規則を正本 1 箇所に畳む（手渡しは前任の `工程完了:` return が条件）
...
- **検査を禁止語の列挙からホワイトリストに変えた**: `tests/handoff-declaration.bats` と `tests/lib/handoff-scan.py`（新規）は「トリガー語に掛かった面が正本への参照を持つか」だけを見る。禁じたい言い回しを列挙する形は、1 つの言い回しに限定した正規表現で 2 度、面を手で列挙した一覧で 1 度、取り残しを見逃した
  ...
  - 語彙 B だけで発火した誤検出は、理由コメント 1 行付きの除外表に載せられる（現在 5 件。モデル ID の世代交代・snapshot の前回値・PR 本文の読者など）。語彙 A の面は載せられず、除外表の各行は当該面が今もトリガー語を含むことを検査して stale を落とす
  - ホワイトリストは「そのファイルが参照を持つか」しか見ないので、参照を持つファイルの中に残った再掲は、正本が規定する①〜④それぞれを文単位で拾う走査で落とす（①だけ・②だけを見ていると同じ欠陥が別の規則で再発する。実際、許可条件（②）しか見ていなかったため SKILL.md の失敗フローに残った再開の禁止（①）の再掲を見逃した）。書式リテラル `工程完了: <工程名>` / `工程中断: <理由>` は spec が固定していて再掲に当たらないので、③の判定前に取り除く。正本が①〜④を実際に答えていることを固定する `criteria:` 系 9 件は残した（spec は答えを再掲しないので、規範の中身を担保するのはこの検査だけになる）
