...
- **検査を禁止語の列挙からホワイトリストに変えた**: `tests/handoff-declaration.bats` と `tests/lib/handoff-scan.py`（新規）は「トリガー語に掛かった面が正本への参照を持つか」だけを見る。禁じたい言い回しを列挙する形は、1 つの言い回しに限定した正規表現で 2 度、面を手で列挙した一覧で 1 度、取り残しを見逃した
  - トリガーは語彙依存であることを前提に再現率優先で取る。語彙 A（`手渡` / `工程完了` / `工程中断` / `subagent-context` / `DEV_WORKFLOW_CONTEXT_CAP` / `コンテキスト上限`）はリポジトリ全体、語彙 B（`引き継` / `交代` / `後任` / `乗り換え` / `新しい W` / `新しい G` / `新規 spawn` / `再 spawn`）は `plugins/dev-workflow/` 配下と dev-workflow の live spec に当てる
  - 検査する面は `git ls-files` からの機械的な列挙で、除外は 3 種だけ（歴史記録＝`plugins/dev-workflow/CHANGELOG.md` 全体・この change 以外の archive・`_longruns/`／change の `proposal.md` と `tasks.md`／`plugin.json`）。除外するのは dev-workflow の CHANGELOG だけで、他プラグインの `CHANGELOG.md` は検査対象に残す。`tests/` 自身も検査対象に入る
  - 語彙 B だけで発火した誤検出は、理由コメント 1 行付きの除外表に載せられる（現在 7 件。モデル ID の世代交代・snapshot の前回値・PR 本文の読者・Bash 呼び出しをまたがないシェル変数など）。語彙 A の面は載せられず、除外表の各行は当該面が今もトリガー語を含むことを検査して stale を落とす
  - ホワイトリストは「そのファイルが参照を持つか」しか見ないので、参照を持つファイルの中に残った再掲は、正本が規定する①〜④それぞれを文単位で拾う走査で落とす（①だけ・②だけを見ていると同じ欠陥が別の規則で再発する。実際、許可条件（②）しか見ていなかったため SKILL.md の失敗フローに残った再開の禁止（①）の再掲を見逃した）。書式リテラル `工程完了: <工程名>` / `工程中断: <理由>` は spec が固定していて再掲に当たらないので、③の判定前に取り除く。正本が①〜④を実際に答えていることを固定する `criteria:` 系は残した（現在 13 件）（spec は答えを再掲しないので、規範の中身を担保するのはこの検査だけになる）
  - 再掲の走査は **spec が列挙した言い回しに対する検査**であって、任意の言い換えを検出するものではない（自由な言い換えを正規表現で網羅的に検出することは原理的に不可能で、追いかけると別の言い回しに抜けられる）。本文が 1 箇所にあることの保証はホワイトリストと仕様レビューが担い、この走査は補助の網。走査の語彙（規則ごとの話題語と規範の述語）は spec が列挙し、実装は決めない。①②③は規範の述語が**文末**に来ることを求めて、名詞句を並べただけの話題の列挙と再掲を分ける。走査の 1 文は「行を `。` で区切ったもの」で、判定前に `【…】` の注記を（③の述語判定だけは書式リテラルも）取り除く — この前処理も spec 側の決定
  - **正本への参照を同じ文に含む再掲も外さない**。参照を文末に添えて規則を再掲する形が最も自然な書き方で、この設計の動機になった実物（修正前の `SKILL.md`）もその形だったため、参照を含む文を外すと穴が一番当たりやすい所に開いていた（実測: 「正本のとおり、…前任に続きを依頼してはならない」も、修正前 `SKILL.md` の②の再掲も無検出だった）。MAY で認めたガード 1 行は正本を読む義務だけを述べる文なので、話題語と規範の述語が同じ文に揃わず掛からない
  - ①〜④の答えを述べていないのに語彙に掛かる文（手渡しと無関係な文、正本の置き場所を定める要件本文そのもの）は、参照への書き換えでは解消できないので〈面のパス・断片・理由コメント 1 行〉の除外表に載せて**文単位**で外す（面単位では外せない。断片が走査に掛からなくなった行は stale として落とす）
  - 検出器の 2 ファイル（`tests/lib/handoff-scan.py` / `tests/handoff-declaration.bats`）の除外は**行単位**（行末に `handoff-scan: fixture` の印を持つ行だけ）。ファイル単位で外していたため、この 2 ファイルには規則を書き足しても検出されない穴があった。発火の確認も 1 規則 1 スキャンにして、あるサンプルが 2 つの規則に掛かる取り違えを落とす。サンプルは規則ごとに、正本の語をそのまま使った文と、正本の語をひとつも使わない言い換えの 2 本
- **検査が「何も見ていないまま緑」にならないようにした**（2026-09-09 のレビュー指摘。実装を壊して全テストが緑になるかで 1 件ずつ確かめた）
  - ③の書式リテラル除去は**述語の判定にだけ**掛ける。話題語と足切りは除去前の文に当てる（spec の明示。1 本の正規表現に入れていたため、話題語がリテラルの中にしか無い文 — 書式を正しく引用した、実際に面へ書かれる形 — が③から抜けていた）
  - `excluded()` を広げれば任意の面を検査から外せた（名指しの 7 面＋2 つの live spec を足しても緑）ので、名指しした参照面と正本が検査対象に入っていることを固定する
  - ホワイトリスト走査に負のコントロールを置いた（参照判定を常に真に、語彙 A を絶対マッチしない正規表現に変えても緑だった）
  - 正本へのパスが実在することを検査する。対象は名指しの 9 面ではなく、トリガー語で発火して参照を持つ面すべて（名指しだけを見ると、あとから参照を足した面のポインタが壊れても素通りする）。`README.md` と `templates/escalation-tripwires.md` は、同じファイルの中で `references/` をプラグイン直下と定義しながら正本を `references/decision-criteria.md` と書いていて、ポインタが解決しなかった
  - 前処理（`【…】` の除去・書式リテラルの除去）が効かないと検出できない形のサンプルを 1 本ずつ持つ（どちらも無効化して緑だった）
  - spec が列挙した語彙の各要素だけで組んだ合成文を走らせ、要素をひとつ消すと落ちるようにした（`工程の終わり` を消しても緑だった）。言い換えサンプルは spec の MUST どおり正本の語をひとつも使わない形に直し、それ自体を機械検査する
  - 正本の節の切り出しを次の見出し（`## ` / `### `）で止める（後続の小節まで含んでいたため、正本の本文が小節へ移動しても `criteria:` 系が緑のままだった）
...
- **停止確認が返らないときの終端は入れない**（2026-09-09 のレビュー B1〜B3）。一度は「本体のターンを 3 回はさんだら人間に手渡しの可否を仰ぐ」と書いたが、①終端の帰結が検査で固定できておらず、事故そのもの（前任を放置して後任を spawn する）を許す文に書き換えても全テストが緑だった、②「人間に可否を仰ぐ」が手渡しの許可条件（工程完了 return か停止確認のいずれかだけ）と矛盾する第 3 の経路になる、③本体のターン数は終端の単位として働かない（他に進める作業が無ければカウンタが進まず、あれば数秒で 3 ターン空回りする）。この論点は別 issue に切り出す
- **走査の語彙をテスト内に写さず live spec から読み取る**ようにした（`工程の終わり` を実装とテストの写しの両方から消しても、spec を残したまま全テストが緑だった）。spec 側の一覧は 1 行 1 グループの機械可読な形（グループ名＋バッククォート区切りの語）に組み直し、読み取れたグループが 13 個に満たなければテストを落とす
