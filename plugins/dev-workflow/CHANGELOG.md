...
- **「再開の禁止」と「手渡しの許可」を分離**: exit 2 のときは前任の状態にかかわらず作業の継続を指示する SendMessage を送らない（無条件）。手渡し（新しい W / G の spawn）は前任の直近 return の 1 行目が `工程完了:` のときだけ。停止を指示する SendMessage はこの禁止の対象外
...
- 書き換え漏れの再発防止として、`tests/handoff-declaration.bats` に否定アサーションを追加した。手渡しの規則を述べる現行面を列挙し、旧文言（`再開せず`／`再開しない` に続けて手渡し・新しいエージェントを述べる形）が 1 か所でも残っていたら落ちる。CHANGELOG と過去の change の archive は歴史記録なので対象外
