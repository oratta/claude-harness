...
- **usage-probe.sh**: snapshot の mtime による 5 分 TTL を外し、記録か snapshot が 3 時間より古いスロットだけを叩く。前回の試行から 3 時間は叩かず、429 が続くスロットは待ちを倍々に延ばす（上限 1 日）。試行状態を `.usage-probe-state` に記録し、`.usage-probe.lock` でマシン全体 1 本に絞る
