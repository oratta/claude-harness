...
## 2.13.27 — 2026-09-23: 仕分け表の順 3 で書き換えた「該当しない」行を補助表で扱い、2 段目を `review-hit-set.py --head` で回す

順 3 の一覧で「該当しない」とした行の本文を同じ周の別の修正で書き換えると、その行の本文は修正前と HEAD で異なるので、表にどちらの本文を書いても 1 段目（修正前 SHA）か 2 段目（HEAD の残存ヒット）の片方が不一致になり、正しく直した PR が差し戻されていた（#377）。main の 2.13.26 は並行して入った変更が使っているため、この版は 2.13.27 とした。
