...
- **検査を禁止語の列挙からホワイトリストに変えた**: `tests/handoff-declaration.bats` と `tests/lib/handoff-scan.py`（新規）は「トリガー語に掛かった面が正本への参照を持つか」だけを見る。禁じたい言い回しを列挙する形は、1 つの言い回しに限定した正規表現で 2 度、面を手で列挙した一覧で 1 度、取り残しを見逃した
  ...
  - 検査する面は `git ls-files` からの機械的な列挙で、除外は 3 種だけ（歴史記録＝`CHANGELOG.md` 全体・この change 以外の archive・`_longruns/`／change の `proposal.md` と `tasks.md`／`plugin.json`）。`tests/` 自身も検査対象に入る
  ...
  - ホワイトリストは「参照があるか」しか見ないので、正本以外の面が許可条件を絶対文で述べていないことを文単位でも固定する。正本が①〜④を実際に答えていることを固定する `criteria:` 系 9 件は残した（spec は答えを再掲しないので、規範の中身を担保するのはこの検査だけになる）
