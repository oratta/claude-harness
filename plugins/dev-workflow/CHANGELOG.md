...
- **検査を禁止語の列挙からホワイトリストに変えた**: `tests/handoff-declaration.bats` と `tests/lib/handoff-scan.py`（新規）は「トリガー語に掛かった面が正本への参照を持つか」だけを見る。禁じたい言い回しを列挙する形は、1 つの言い回しに限定した正規表現で 2 度、面を手で列挙した一覧で 1 度、取り残しを見逃した
  ...
  - 再掲の走査は **spec が列挙した言い回しに対する検査**であって、任意の言い換えを検出するものではない（自由な言い換えを正規表現で網羅的に検出することは原理的に不可能で、追いかけると別の言い回しに抜けられる）。本文が 1 箇所にあることの保証はホワイトリストと仕様レビューが担い、この走査は補助の網。走査の語彙（規則ごとの話題語と規範の述語）は spec が列挙し、実装は決めない。①②③は規範の述語が**文末**に来ることを求めて、名詞句を並べただけの話題の列挙と再掲を分ける。同じ文が正本への参照を含むならポインタ文として拾わない
