...
## 2.5.1 — 2026-09-08: 手渡しは前任の `工程完了:` return が条件
...
- 同じ規則を要約している面をすべて追随させた（レビュー指摘）: SKILL.md の 1 ループ (3)(4)、`worker.md` の節冒頭、`README.md`、`scripts/session-tripwires.sh` の毎セッション注入文、`scripts/subagent-context.sh` のヘッダコメント、`plugin.json` の description、`openspec/specs/dev-workflow-develop` の「役割のモデル」要件（`dev-workflow-execution-strategy` と重複していた要約）
- 書き換え漏れの再発防止として、`tests/handoff-declaration.bats` に否定アサーションを追加した。手渡しの規則を述べる現行面を列挙し、旧文言（`再開せず`／`再開しない` に続けて手渡し・新しいエージェントを述べる形）が 1 か所でも残っていたら落ちる。行単位の走査では取りこぼす形（「コンテキスト上限は昇格ではなく手渡しで」のように「再開」の語を含まない言い回し）があったため、文単位の走査を 2 件足した: 上限超過と手渡しを同じ文で述べるのに条件を書いていない文を落とすものと、「手渡しを行ってよいのは」を含む文に `工程完了` と `停止確認` の両方を要求するもの。後者の検査対象は面の手入力ではなく `git ls-files` からの機械的な列挙にした（面を手で挙げると、まだ直していない面が検査から外れる）。CHANGELOG と過去の change の archive は歴史記録なので対象外
