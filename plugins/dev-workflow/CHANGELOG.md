...
- **検査が「何も見ていないまま緑」にならないようにした**（2026-09-09 のレビュー指摘。実装を壊して全テストが緑になるかで 1 件ずつ確かめた）
  ...
  - 参照面に書かれた正本へのパスが実在することを検査する。`README.md` と `templates/escalation-tripwires.md` は、同じファイルの中で `references/` をプラグイン直下と定義しながら正本を `references/decision-criteria.md` と書いていて、ポインタが解決しなかった
...
- **正本に 2 点足した**: 固定された 2 つの書式のどちらにも当てはまらない return は `工程中断:` と同じに扱う（展開直後に必ず現れる形で、明記が無いと本体が内容判断へ戻る）。停止の指示に応答が返らないままのときの終端（interactive も無制限には待たない）。あわせて④の「前任が先に `工程完了:` を返したら、そのまま続行し手渡しはしない」が①②と矛盾していたのを、通常の手渡しに戻る形に直した
