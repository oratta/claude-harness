...
- 仕分け（2周目と、主の続行指示で開いた周の return で必須。Status によらず書く）: pr-review-gate 収束ルールの「2周目の終わりにやること」の仕分け結果（指摘ごとに違反文の引用、または follow-up issue の URL）と、その PR コメント URL。全件を follow-up issue に切って passed で返すときもここに書く。全周共通の判定で止める指摘が残っている場合は failed ではなく保留で返す（下）
### passed のとき
...
### 保留のとき
- 2周目キャップ（2周目の終わり、または主の続行指示で開いた周の終わりに、全周共通の判定で止める指摘が残った）: 引用した違反文と対応する指摘、主に依頼する1択（続けるか、範囲外として閉じるか）。W の再開や3周目を提案しない（決める役も挟まない）
