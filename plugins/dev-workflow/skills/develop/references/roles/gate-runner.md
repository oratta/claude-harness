...
- 仕分け（2周目と、主の続行指示で開いた周の return で必須。Status によらず書く）: pr-review-gate 収束ルールの「2周目の終わりにやること」の仕分け結果（指摘ごとに違反文の引用、または follow-up issue の URL）と、その PR コメント URL。全件を follow-up issue に切って passed で返すときもここに書く。引用できる指摘が残っている場合は failed ではなく保留で返す（下）
### passed のとき
...
### 保留のとき
- 2周目キャップ（2周目の終わり、または主の続行指示で開いた周の終わりに、引用できる指摘が残った）: 引用した違反文と対応する指摘、主に依頼する1択（続けるか、範囲外として閉じるか）。W の再開や3周目を提案しない（決める役も挟まない）
...
- **レビュアーの要約受領**: 上の「レビュー実行者:」コメントを投稿してから、要約に指摘が残っているかで分岐する。指摘が無ければ手順 3 以降。指摘が残っていれば、1周目は pr-review-gate 手順 2 の一般則（欠陥ありなら `agent-review:failed` に付け替えて failed で返す）を、2周目と主の続行指示で開いた周は収束ルールの「2周目の終わりにやること」の仕分けを通してから分岐する（引用できない指摘だけなら follow-up issue に切って手順 3 以降、引用できる指摘が残れば保留）
