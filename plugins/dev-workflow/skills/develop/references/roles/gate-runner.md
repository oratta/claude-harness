...
- 周回: <1|2|3以降（主の続行指示あり）>（全体レビューにした周は「（全体レビュー: 修正差分 N 行 / 前周指摘 M 行）」を添える）
- 仕分け（2周目と、主の続行指示で開いた周の return で必須。Status によらず書く）: pr-review-gate 収束ルールの「2周目の終わりにやること」の仕分け結果（指摘ごとに違反文の引用・例外 3 種のどれか（安全機構の穴・データ破壊・無言の機能不全）、または follow-up issue の URL）と、その PR コメント URL。全件を follow-up issue に切って passed で返すときもここに書く。全周共通の判定で止める指摘が残っている場合は failed ではなく保留で返す（下）
### passed のとき
...
### 保留のとき
- 2周目キャップ（2周目の終わり、または主の続行指示で開いた周の終わりに、全周共通の判定で止める指摘が残った）: 引用した違反文（または例外 3 種のどれか）と対応する指摘、主に依頼する1択（続けるか、範囲外として閉じるか）。W の再開や3周目を提案しない（決める役も挟まない）
...
- **W の修正後の再レビュー**: 手順 1 から全工程をやり直す（`failed` を外して `pending` に戻す）。再レビューの範囲は差分限定。ただし W の修正が方式の書き換え（修正差分の行数が前周の指摘の対象行数を大きく超える）なら全体レビューにし、2 つの行数を PR コメントに記録する。周回は数え続ける（2 周キャップ。3 周目に入るのは主の続行指示があったときだけ）
- **レビュアーの要約受領**: 上の「レビュー実行者:」コメントを投稿してから、要約に指摘が残っているかで分岐する。指摘が無ければ手順 3 以降。指摘が残っていれば、pr-review-gate 手順 2-1 の全周共通の判定を通してから分岐する。止める指摘が無ければ follow-up issue に切って手順 3 以降。止める指摘があれば、1周目は `agent-review:failed` に付け替えて failed で返し（止めない指摘は failed の PR コメントに一覧で残す）、2周目と主の続行指示で開いた周は収束ルールの「2周目の終わりにやること」の仕分けを PR コメントに記録して保留で返す
- **保留の解除**: オーナーの回答（許容する／しない・動作確認の結果・2周目キャップへの回答（続ける／範囲外として閉じる））を受けて pr-review-gate の復帰手順に従う
