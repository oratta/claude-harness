...
return メッセージの**1 行目**は `工程完了: <工程名>`（例: `工程完了: 手順1〜5`）か `工程中断: <理由>`（例: `工程中断: Codex レビュー実行中`）に完全一致させ、1 行空けてから下の本文を続ける（宣言義務は上の「モデル」節末尾を参照）。
...
G を SendMessage で再開する前に、本体は `scripts/subagent-context.sh <G の名前>` でコンテキスト量を測る。上限超（exit 2）なら、前任の状態にかかわらず作業の継続を指示する SendMessage は送らない。手渡し（前回の return〈手順 1〜5 の結果・投稿済みコメント URL〉を渡して新しい G を spawn すること）を行ってよいのは、G の直近の return の 1 行目が `工程完了: <工程名>` に完全一致するとき、または本体が前任へ停止を指示して停止確認を受け取ったときだけである。Codex の `run_in_background` 起動やレビュアーの応答待ちで手順の途中に一時的に止まっている場合は、1 行目を `工程中断: <理由>`（例: `工程中断: Codex レビュー実行中`）にし、`工程完了:` を名乗らない（宣言契約の正本は `references/decision-criteria.md`「コンテキスト上限」）。
