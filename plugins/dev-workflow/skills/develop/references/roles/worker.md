...
**G から一覧を求められた指摘（仕分け表の順 3・順 6 の「全部列挙してから直す」）**: 検索コマンド（grep の語、または場合分けの軸とその全域）と、全ヒットごとに「直した／該当しない理由」を付けた表を PR コメントに投稿してから push する（G はその検索コマンドを自分で実行して集合の一致だけを見るので、投稿が push より後だと照合できない）。書式の正本は pr-review-gate の SKILL.md 手順 2-1 の仕分け表の順 3 で、ここには再掲しない。
