...
W は再開のたびに全履歴を読み直すので、履歴は畳まれずに伸び続ける。本体は W を SendMessage で再開する前に毎回 `scripts/subagent-context.sh <W の名前>` で測る（exit 2 が上限超）。加えて、W 自身の起動の途中でも hook がコンテキストを測り、上限を超えると締めを通知し、さらに越えると `Edit` / `Write` / `NotebookEdit` と `Bash`（コマンド内容によらず全件）を拒否する。強制停止中は commit もできないため、未コミット差分の後片付けは本体が行う。閾値と全解除の環境変数・2 経路・通知や強制停止に当たったときの振る舞いは正本にある。
