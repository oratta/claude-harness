...
W は再開のたびに全履歴を読み直すので、履歴は畳まれずに伸び続ける。本体は W を SendMessage で再開する前に毎回 `scripts/subagent-context.sh <W の名前>` で測る（exit 2 が上限超）。加えて、W 自身の起動の途中でも hook がコンテキストを測り、上限を超えると締めを通知し、さらに越えると編集系ツールを拒否する。閾値と全解除の環境変数・2 経路・通知や強制停止に当たったときの振る舞いは正本にある。
