...
- **強制停止に当たったら**（`DEV_WORKFLOW_CONTEXT_HARD_CAP` 超）: 残せるのは commit と return だけ。`Bash` は「受理する文法に照合して読める形だけを通す」方式で判定する（正の列挙。表に無いオプションは危険と分かっていなくても拒否）: `git` の `status` / `diff` / `add` / `commit` / `push` だけが対象で、作業ディレクトリの指定は `-C <path>` だけが通り、`-c <k=v>` は値によらず一律拒否される。サブコマンド以降はそのサブコマンドの許可オプション表にある形・値オプション（`-m` 等）直後の自由な値・`-` で始まらず `::` を含まないオペランドの 3 通りだけを通す（表は `openspec/specs/dev-workflow-execution-strategy/spec.md`「強制停止の閾値を超えたら PreToolUse が編集を拒否する」が正本）。パイプ・`&&`・`;`・サブシェル・コマンド置換 `$(…)` を含むコマンドは拒否される（commit メッセージは `-m` を複数回に分けて 1 行ずつ渡す）。読み取り系（Read / Grep / Glob）は拒否されない
