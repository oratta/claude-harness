...
- **停止確認が返らないときの終端**: 前任がツール実行中で SendMessage を読まないことがあるので、interactive も無制限には待たない。本体が自分のターンを 3 回はさんでも停止確認が返らなければ待つのをやめ、前任の名前・停止を指示した時刻・その worktree の `git status` を添えて人間に報告し、手渡しの可否を仰ぐ（前任を強制終了させない。手渡し先を先に spawn しない）。unmanned は上のとおりそのサイクルを終える
