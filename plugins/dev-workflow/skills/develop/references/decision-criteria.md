...
- **手渡しの許可（前任の `工程完了:` return が条件）**: 手渡し（前回の return と記録先を渡して**同じ役割の新しいエージェント（W なら新しい W、G なら新しい G）を spawn する**こと）を行ってよいのは、前任の直近の return の 1 行目が `工程完了: <工程名>` に完全一致するときだけ。上限超過（exit 2）は「次に再開するときは手渡しに切り替える」という条件であって、「今すぐ交代させる」条件ではない。手渡し先は前任の return を前提に続け、記録先とファイルの現状から再出発する（前任の履歴は読めないし読まない）
- **W / G 側の宣言義務**: return の 1 行目を次のどちらかに完全一致させる（太字・全角コロン・末尾句点を付けない。書式は `仕様化判断: する|しない` と同型。正本は `references/roles/worker.md`）:
...
- **idle と return の違い**: バックグラウンドコマンドの完了を待って一時的に応答が止まっている（idle）状態は工程の終わりではない。そのコマンドが終われば前任は再び動き出す。本体は上の宣言（`工程完了:` か `工程中断:` か）だけで見分け、内容を読んで独自に判定しない
...
- **停止確認を待つ間**: 本体はブロックせずに待つ。他に進められる役割（別 worktree の並列作業）があれば先に進めてよい。前任が停止確認より先に `工程完了:` で return したら、そのまま続行し手渡しはしない。unmanned（1 サイクル 1 仕事）で他に進められる作業が無ければ、そのサイクル内で停止確認を待ち続けず、そのサイクルを終える（次サイクルで同じ判定をやり直す）
