...
**順 3（一覧の一致で閉じる）**: W は、検索コマンド（grep の語、または場合分けの軸とその全域。例: 先頭バイトなら 0x00〜0xFF）と、全ヒットに「直した／該当しない理由」を付けた表を PR コメントに投稿してから push する。G はレビューをせず、同じ検索コマンドを自分で実行して、ヒットの集合が W の表と一致するかだけを見る（「該当しない理由」の正否を判定しない）。表に無いヒットがあれば、その差分だけを W に返す。差し戻しは 1 回までで、2 回目も一致しなければ順 6 に落とす。検索コマンドや軸の全域を W が書けない指摘は順 3 に当てない。順 3 の照合はレビューの周に数えない（G はレビューをしないので、周回キャップとは別に差し戻しの上限で止める）。同じ failed で戻した指摘が順 3 だけなら、集合が一致して閉じたあとに差分限定の再レビューは行わず、周を消費しない（閉じた PR コメント URL を仕分け欄に残す）。順 2・順 4 の指摘が同じ failed に含まれていれば、それらの差分限定の再レビューは通常どおり行い、周を 1 つ消費する。
...
| **切り出しの確認**（手順2-1の仕分け表の順 5） | 切り出す | その指摘を follow-up issue に切って URL を PR コメントに記録する → **`needs-approval` を外す** → ほかに止める指摘が無ければ手順3以降へ |
| | この PR で直す | 主の回答リンクを PR コメントに記録する → **`needs-approval` を外す** → `agent-review:failed` に付け替えて修正サイクルへ戻す（2周目以降なら次の周に入る。その周の終わりにも仕分け表を適用する） |
