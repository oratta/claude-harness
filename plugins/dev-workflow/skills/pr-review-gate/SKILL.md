...
- **資格情報**: ID・パスワード・API の鍵・トークンの取得・保管・利用・受け渡しをするコードや設定を足す・変える。秘密の値そのものを含まない文書上の言及だけなら当たらない
- **安全ゲートの弱体化**: レビュー必須・投稿前チェック（hook）・auto-merge の判定条件・permission の deny・ブランチ保護などの止め具を外す・条件を緩める・迂回路を作る。止め具を増やす・厳しくする変更は当たらない
- **エージェント権限の拡張**: permission の allow の追加、新しい外部サービス・MCP・ツールの利用、新しい書き込み先（リポ・ラベル・チャネル・ファイルの置き場）など、エージェントにできることを増やす
