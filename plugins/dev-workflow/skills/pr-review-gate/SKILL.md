...
- **待ち方は読み手で変わる**（詳細の正本は `plugins/dev-workflow/references/subagent-waiting.md`）:
  ...
  ```
  `--timeout-ms` の既定は **4 分**しかないので必ず明示する。値は Bash 前景の上限（600000 ms）未満にする — 従来の 900000 は 1 回の呼び出しで完走せず、これが待ちの構造を壊す原因だった。1 回で終わらなければ**同じ呼び出しをもう一度発行する**（1 回の Bash 呼び出しはターンの終わりではない）。総待ちの上限は前景ループ **3 回（27 分）** で、ここに達したら上の表のフォールバック（Task サブエージェント）に入る。フォールバック条件に挙げた「タイムアウト」はこの上限に達したことを指す。
