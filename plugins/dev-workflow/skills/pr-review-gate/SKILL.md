...
- 場所: `file:line-range`（diff と重なる範囲で 10 行以内）
...
**順 3（一覧の一致で閉じる）**: W は、修正前 SHA・検索コマンド・全ヒットに「直した／該当しない理由」を付けた表を 1 つの PR コメントに投稿してから push する。修正前 SHA は、W が修正に着手する直前の HEAD（G が `agent-review:failed` を付けた周の対象 HEAD）の 40 桁フル SHA とする。一覧表の PR コメントの書式はここを正本とする（worker.md と照合表はこれに揃える）:
- 1 行目 `## 一覧（順 3）`、続けて `修正前 SHA: <40 桁>` の行と `検索コマンド: <コマンド>` の行を置き、その下に表を置く
- grep の語のときは、検索コマンドを `git grep -n <語の指定> <rev> -- <パス>` の形で `<rev>` を含めて書く（G が `<rev>` を修正前 SHA と HEAD に置き換えて実行する）。作業ツリーに対する `grep -rn` の形では書かない（G の作業ツリーがどのコミットを指すかで結果が変わるため）。表の列は `| ファイル | 行（修正前 SHA） | ヒットした行の本文 | 扱い |`
- 場合分けの軸のときも `修正前 SHA:` の行は同じく書き、`検索コマンド:` の行には軸とその全域（例: 先頭バイトなら 0x00〜0xFF）を書く。表の列は `| 軸の値 | 扱い |`
