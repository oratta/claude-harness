...
| failed の原因 | 戻し方 |
|---|---|
| **実装品質起因**（テスト不足・エッジケース漏れ・既存コードとの不整合・安全機構の穴） | 修正実装を**前回の実装モデルの 1 段上**（`sonnet` → `opus` → `fable`。残量モードと共有枠モードの上限内）で spawn する。2 周目が最終周なので、opus で書いた実装が実装品質で落ちれば 2 周目は `model: fable` になる |
...
1周目からモデルを上げる「重要実装」の事前分類（聖域パス・マージ権限・層間契約・課金/法務。聖域パスは `opus`、残り 3 つは `fable`）は **develop スキルの references/roles/worker.md が正本** — このゲートでは再掲しない。
