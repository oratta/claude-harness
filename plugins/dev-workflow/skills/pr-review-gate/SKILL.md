...
**共通一覧契約（`照合表` と仕分け表の順 3 の grep の語の経路の第 1 段）**: `修正前 SHA: <40 桁>`、`検索コマンド: git grep -n <語の指定> <rev> -- .`、`| ファイル | 行（修正前 SHA） | ヒットした行の本文 | 扱い |` を必須とする。検索起点はリポジトリルートで固定 revision の全追跡対象パスを含め、サブディレクトリへの限定や追跡対象パスの除外を禁止する。一周目は固定した対象 HEAD を修正前 SHA とし、第 4 列を `一致` / `食い違い: <finding ID>` にする。順 3 は W が修正に着手する直前の HEAD を修正前 SHA とし、第 4 列だけを `直した` / `該当しない: <理由>` に変える。両経路とも `plugins/dev-workflow/scripts/review-hit-set.py` で第 4 列を除く全ヒット集合を照合する。順 3 固有の修正後 HEAD に対する本文・件数・削除行の第 2 段は別に維持する。順 3 の場合分けの軸は共通一覧契約の対象外で、`検索コマンド:` に軸と全域、`| 軸の値 | 扱い |` を書き、軸の全域との一致だけを検査する。
