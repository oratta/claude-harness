...
再レビュー（差分限定）は、前回の指摘ごとに状態を付けて閉じたかを確かめるだけにし、新規の指摘を出さない（新規の `nit` も出さない）。方式の書き換えのあとに PR 全体を見直す周でも、新規に出してよいのは `blocking` だけ。前回の「直し方」どおりに直した箇所を再指摘しない。直し方自体が誤っていたなら、レビュアー側の誤りとして前回の指摘を `wontfix` にし、理由を書く。
```
...
止める指摘が残ったら、周によって変わるのは動き方だけ（判定は「マージを止めるかの判定（全周共通）」のまま）。1周目は PR コメントに止める指摘を書き、`agent-review:failed` に付け替えて修正サイクルへ戻す（ここで終了）。2周目と主の続行指示で開いた周は、下の収束ルールの「2周目の終わりにやること」に従う。
...
- レビューは**既定2周**（初回レビュー＋再レビュー1回）で確定する。**3周目を自動で開けない**（G も本体も開けない）。
...
3. **止める指摘が1件でも残ったら、3周目に入らず止まる**（引用できる指摘でも、判定で止まらないものは上の follow-up issue 側に回す）。PR に `needs-approval` を付け、引用した文（または例外 3 種のどれか）と対応する指摘を添えて、主に「**続けるか、範囲外として閉じるか**」の1択を出す（手順6の保留として扱い、復帰は手順6の表の「2周目キャップ」行に従う）。無人運用（loop-dev-agent）でも同じく止まる。3周目は主が「続ける」と答えた場合にだけ開く。**主の続行指示で開いた周の終了時にも同じ仕分けを適用し**、止める指摘が残れば再び `needs-approval` で止まる（主の回答なしに次の周へ進まない）。

**決める役（`dev-workflow:decider`）はキャップの判定に関与しない。** 決める役の裁定は手順2-2の既存の出力契約（原因分類・直し方・次のモデル）のままで、上の仕分けと3周目に入るかどうかは、全周共通の判定で止める指摘が残るかどうかで決まる。キャップで主に上げるときに決める役を挟まない。
...
前提として、**最後のレビュー結果に、全周共通の判定で止まる指摘（`blocking` かつ `confirmed`、G が引用を照合済みまたは例外 3 種に当たるもの）のうち、主が範囲外として閉じて follow-up issue に切ったもの以外が 0 件であること**（判定は手順2-1「マージを止めるかの判定（全周共通）」。範囲外として閉じる経路は手順6の表の「2周目キャップ」行）。そのうえで、**手順4の動作確認証拠を投稿済み、かつ手順3のリスク宣言が次のどちらかの状態のときだけ**実行する。
...
| **2周目キャップ**（手順2の収束ルール） | 続ける | 主の回答リンクを PR コメントに記録する → **`needs-approval` を外す** → `agent-review:failed` に付け替えて修正サイクルへ戻し、周回3以降に入る（その周の終わりにも同じ仕分けを適用する） |
| | 範囲外として閉じる | 引用できた指摘も follow-up issue に切って URL を PR コメントに記録する → **`needs-approval` を外す** → 手順3以降へ |
...
| # | 分類 | 具体 |
|---|---|---|
| 1 | リスク許容の確認 | 手順3。実装中に発覚したものも発覚時点で確認する。手順2の2周目キャップ（引用できる指摘を残して続けるか、範囲外として閉じるか）もここに入る |
