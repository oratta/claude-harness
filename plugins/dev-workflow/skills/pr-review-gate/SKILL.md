...
| フォールバック | Task サブエージェント | Agent ツール（`general-purpose`）に受け入れ条件＋diff 範囲を渡す | ① **light** と判定したとき（最初からこれ）② full だが Codex CLI が使えないとき（実測したバイナリ無し・認証切れ・タイムアウト） |
...
- **exec のレビュー指示はファイルに保存して標準入力から渡す。** 固定した HEAD・diff 範囲・受け入れ条件を含め、起動・完了シグナル・結果の読み方は待ち方の正本に従う。
...
欠陥ありなら PR コメントに再現手順と修正点を書き、`agent-review:failed` に付け替えて修正サイクルへ戻す（ここで終了）。この一般則は1周目に適用する。2周目と主の続行指示で開いた周の終わりは、下の収束ルールの「2周目の終わりにやること」が優先する（深刻度によらず、引用できるかどうかで仕分けてから分岐する）。
...
- **blocking の定義を限定する**: マージ後に issue で直せるものは blocking にしない。follow-up issue 化してマージを通す。この定義は、2周目の終わりに次の手順で適用する。

**2周目の終わりにやること（G が、2周目の結果を受け取った直後に）**:

1. **残った指摘ごとに、違反する文を引用する。** 引用元は記録先（issue または Draft PR 本文）の受け入れ条件と、この PR が触れる openspec の spec（change の delta spec を含む）に限る。blocking かどうかは引用できるかどうかで決め、**レビュアー（Codex・Task サブエージェント）の深刻度ラベルは参考にとどめる**（「高深刻度」と付いていても、引用できなければ blocking ではない）。**引用元が無い**とき（PR が openspec に触れず、記録先にも受け入れ条件の文が無い）は、引用元を広げず、残った指摘を全件「引用なし」として扱う。仕分けの結果（指摘ごとの引用、または「引用なし」）を PR コメントに記録する。
2. **引用できない指摘は follow-up issue に切って `passed` の判定へ進む。** 各指摘を follow-up issue にし、URL を仕分けの PR コメントに記録して、手順3以降へ進む。引用できない指摘を理由に3周目を開けない。
3. **引用できる指摘が1件でも残ったら、3周目に入らず止まる。** PR に `needs-approval` を付け、引用した文と対応する指摘を添えて、主に「**続けるか、範囲外として閉じるか**」の1択を出す（手順6の保留として扱い、復帰は手順6の表の「2周目キャップ」行に従う）。無人運用（loop-dev-agent）でも同じく止まる。3周目は主が「続ける」と答えた場合にだけ開く。**主の続行指示で開いた周の終了時にも同じ仕分けを適用し**、引用できる指摘が残れば再び `needs-approval` で止まる（主の回答なしに次の周へ進まない）。

**決める役（`dev-workflow:decider`）はキャップの判定に関与しない。** 決める役の裁定は手順2-2の既存の出力契約（原因分類・直し方・次のモデル）のままで、上の仕分けと3周目に入るかどうかは引用の有無で決まる。キャップで主に上げるときに決める役を挟まない。
