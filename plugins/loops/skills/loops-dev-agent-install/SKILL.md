...
あわせて空の `docs/agent-loop-log.md`（実行ログのテーブルヘッダのみ）と、
`docs/GUARDRAILS.md`（「このプロジェクトで踏んだ地雷と回避策を1行ずつ追記する」という説明だけの初期状態）を作成する。
...
見つかったら、issue への移行を提案する。ユーザーが承認したら:

- 未完了タスクを 1 タスク 1 issue で起票する（Step 3 のテンプレート構造に整形。
  受け入れ条件が書かれていないタスクは本文から推定し、推定である旨を明記する）
- タグをラベルにマップする: 【自律可】→ `agent-ready`、【承認待ち】→ `needs-approval`、
  【人間】→ `human-only`
...
6. レビュー段階の効果測定: `docs/agent-loop-log.md` のレビュー検出率が数週間ほぼ 0% なら
   レビューモードを外す判断ができること
