---
name: run-verifier
description: 静的検証を行う。品質（テスト+lint+型チェック+ビルド）と完成度（エッジケース・エラーハンドリングのコードレビュー）を評価する。ブラウザ検証はrun-browser-verifierが担当。
...
ブラウザ動作確認（機能性・UX）は **run-browser-verifier** が別途担当する。
