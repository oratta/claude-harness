...
# 自律実行中に {run-dir}/checkpoint.md のタイムスタンプを更新するスクリプト
# Hook (Stop イベント) から呼び出される

# 最新のランディレクトリを特定
RUN_DIR=$(ls -1d _runs/20*/ 2>/dev/null | sort | tail -1)

if [ -z "$RUN_DIR" ]; then
  exit 0
fi

CHECKPOINT_FILE="${RUN_DIR}checkpoint.md"
...
echo "Checkpoint updated at $TIMESTAMP in $RUN_DIR (latest commit: $LATEST_COMMIT)"
