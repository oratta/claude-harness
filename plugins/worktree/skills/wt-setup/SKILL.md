...
1. **`--with-pr` フラグ**: worktree 作成と同時に空 commit → push → Draft PR を作る。作業を早い段階から remote 側に見える状態にする用途。Claude Code プラグインの配布元リポジトリ（claude-harness）のように、PR 経由のマージを必須にしている repo で使う想定。
...
これは作業中の Draft PR です。作業を早い段階から remote 側に見える状態にする目的で自動作成されました。
...
git -C <clone> worktree add <worktree-path> ${BRANCH}
cd <worktree-path>
# Claude Code を立ち上げて /wt-setup で開発環境復元
\\\`\\\`\\\`

push 済みの commit は remote から復元できる。session.jsonl のような ephemeral ファイルは復元対象外。
