...
3. **配下にプロセスが稼働中／当日のセッションログがある／worktree がロックされている場合は自動処理しない（稼働シグナル）**
   ...
   **配下プロセスの例外 — 居残り（孤児）プロセスは稼働シグナルに数えない**: 2026-08-10 に flatmate リポで全 11 件を実行したとき、**Pass 1 で自動処理できたのは 0 件**で、squash マージ済み・clean な 5 件を含む全件が判断バッチへ流れた。原因は、worktree ごとに開いた codex/claude セッションを閉じてもプロセスがデタッチして残り、`lsof +D` がそれを 7 日間「作業中の証拠」と読み続けたことにある（先頭プロセスの `PPID=1`、状態は `S` のまま）。そこで次の **2 条件を両方**満たすプロセスだけは稼働シグナルから外す:
   ...
   **この AND を崩してはならない（SHALL NOT）**。片方だけで居残りと判定すると、issue #77 のガードが壊れる。1 だけで判定すると `launchd` 常駐の住人セッション（正当に `PPID=1`）を消し、2 だけで判定すると「24 時間黙っているだけの対話セッション」を消す。逆に issue #77 で消してしまった壁打ちセッションは**親プロセスが生きていた**ため、条件 1 で引き続き守られる。判定に必要な情報が取れないとき（`ps` が引けない・ヘルパ未定義・セッションログ検査不能）は**居残りと断定せず稼働シグナルとして扱う**（fail-closed）。

   除外したプロセスは黙って捨てず、**Pass 1 の診断表示・Pass 2 の提示・Step C のレポートに「居残りと判定した根拠」を必ず残す**（PID・コマンド名・親セッション消滅・痕跡なしの旨）。
...
**一過性の誤検出（false positive）は許容する**。見逃し（false negative）の最悪は「作業中セッションの永久消失」なので、判定は常に**検出する側に倒す**。

**ただし「誤検出のコストは Pass 2 で 1 問聞かれるだけ」という見積もりが成り立つのは、シグナルがいずれ消える場合に限る。** 恒久的に立ちっぱなしのシグナルでは、コストは「その worktree が**永久に自動処理されない**」に変わる。2026-08-10 の flatmate では 11 件中 Pass 1 で処理できたものが 0 件になり、ガードが効いているのではなく自動化が死んでいる状態になった。居残りプロセスの除外（上の `detect_active_procs_under` の 2 条件 AND）はこの一点のためにあり、それ以外の誤検出は従来どおり許容する。**除外条件を片方だけに緩めてはならない**（SHALL NOT — issue #77 のガードが壊れる）。
...
- 居残り（孤児）プロセスとして稼働シグナルから外した対象がある場合、除外根拠（PID・コマンド名・親セッション消滅・24 時間以内の更新なし）を表示したことを確認する。**除外は 2 条件 AND を満たした対象に限る**（親が生きているプロセス・24 時間以内にセッションログや worktree が更新されている対象を除外していないこと）。
