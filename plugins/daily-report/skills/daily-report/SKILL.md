...
- **対象日のデフォルト**: `$ARGUMENTS` に日付が指定されておらず、かつ AskUserQuestion が使用できない場合はデフォルト対象日「昨日」を採用して処理を続行する（Step 0 の既存ロジックと同一の `date -v-1d` フォールバックを使う）。質問はスキップする
- **対話依存ステップの代替**: 本スキルは通常 AskUserQuestion を使用しないが、Fieldy/LLM ログの取得が `fail`（`notion-mcp-unavailable` 以外）した場合に本来ユーザーへ口頭確認していたであろう内容は、diary.md の「自分用メモ」セクション内にプレースホルダーとして出力し（例: `> [!warning] 未取得: <理由>`）、対話でユーザーに確認しようとしない
- **判断ログ**: 非対話実行によりデフォルト対象日を採用した場合、diary.md の frontmatter 直後・「## ひとことで」の前に判断ログの callout を追記する:
  ```markdown
  > [!info] 判断ログ
  > 非対話実行（cron）のためデフォルト対象日「昨日」を採用
  ```
  対話実行時（`$ARGUMENTS` で日付が明示された、または AskUserQuestion が利用可能な通常フロー）はこの callout を出力しない
