...
description: ロングラン完了後のフィードバックを分類・実行する。ユーザーの無選別フィードバック（brain dump）をTier 1（cosmetic）/ Tier 2（spec-aligned fix）/ Tier 3（new change）に自動分類し、Tier 1/2は即座に修正、Tier 3はopenspec/backlog.mdに記録する。orchestratorのPhase 4cからも呼ばれる共通ロジック。
version: 1.0.0
...
ロングラン完了後のフィードバックを処理するスキル。
orchestratorのPhase 4c「修正」パスと、`/longrun:feedback` コマンドの両方から使われる共通ロジック。
...
- **処理**: `openspec/backlog.md` に記録。現longrunではやらない
...
1. **未アーカイブlongrunの特定**:
   ```bash
   ls -1d _longrun/20*/ 2>/dev/null | sort | tail -1
   ```
   見つからない場合はエラー: 「アクティブなlongrunが見つかりません」

2. **必要ファイルの読み込み**:
   - `{run-dir}/instruction.md` — スコープ・受け入れ条件の把握
...
「OK」の場合 → `/longrun:archive` を案内（またはorchestratorならPhase 4dに進む）。

## orchestratorからの呼び出し

Phase 4cでユーザーが「修正: [内容]」と応答した場合、orchestratorはこのスキルのStep 2以降を実行する。
