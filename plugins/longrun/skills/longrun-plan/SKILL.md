...
`references/model-tiers.md` が 1 箇所で解決する）:
...
- [ ] 「受け入れ条件」セクションがあるか
- [ ] 「動作確認方法」セクションがあるか
- [ ] 「モデル割り当て」セクションがあるか（表ヘッダ + 各 change × ロールの行が埋まっているか）

<GATE>
「モデル割り当て」セクションが欠落している場合は、保存前に Step 5c のヒューリスティクスで生成して
追加してから保存すること。セクションを欠いたまま plan.md を保存してはならない。
</GATE>
...
**モデル割り当て表の上書き案内（必須）:** ユーザーに、plan.md の「モデル割り当て」表は直接編集して
上書きできることを案内する。ティア欄を書き換える、または `上書き` 欄にティア（haiku/sonnet/inherit）を
記入すると、exec がその値を `opts.model` に反映する（`上書き` 欄がティア欄より優先される）。

<GATE>
ユーザーがモデル割り当て表を直接編集した場合、その編集後の値をそのまま確定すること。
推奨値への巻き戻しや表の再生成を行ってはならない。
</GATE>
