...
Brain Dumpの分析、ユーザーとの対話、instruction.mdの生成、いずれも禁止。
まずテンプレートを読み込み、出力に必要なセクション構造を把握すること。
</HARD-GATE>

**必ずReadツールで以下の2ファイルを読み込む:**

1. `templates/instruction-template.md`（パス: 同プラグイン内の `templates/instruction-template.md`）
   - **これが instruction.md の必須セクション構造を定義する**
   - 全セクション名を抽出し、生成時のチェックリストとして保持する
2. `longrun-orchestrator` の SKILL.md を読み込む（パス: 同プラグイン内の `skills/longrun-orchestrator/SKILL.md`）
   - Phase 0-4 の流れを把握する
...
<HARD-GATE>
このチェックに全て合格するまで、instruction.md をファイルに保存してはならない。
1つでも欠けていたら、instruction.md の内容を修正してから保存すること。
