...
- **「OK」** → Phase 4dへ（アーカイブ）
- **それ以外（フィードバック）** → **feedback-handler スキルに委譲**:
  - feedback-handler の Step 2（アイテム分解）以降を実行
  - Step 0（コンテキストロード）はスキップ（orchestratorが既にコンテキストを保持）
  - Tier 1/2 の修正が完了し、ユーザーが「OK」と応答するまでフィードバックループを継続
  - feedback-handler から制御が戻ったら Phase 4d へ

**フィードバックループの上限**: 最大3ラウンド。3ラウンドを超えた場合、残りのフィードバックは全てTier 3としてbacklogに記録し、アーカイブに進む。
