...
1. ランディレクトリを特定する:
   ...
   - 注: `_longruns/_archive/` 内のアーカイブ済みランは自動検索対象外

2. `{longrun-dir}/checkpoint.md` を読み込む
   - 存在しない場合: 「自律実行は開始されていません。`/longrun:exec` で開始してください」と表示
...
6. アーカイブ済みラン:
   - `_longruns/_archive/` 内のディレクトリを一覧表示
