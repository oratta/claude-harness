...
longrun-planner スキルを使用して、自律実行用の plan.md を対話的に作成してください。

## ⚠️ 必須: ディレクトリ構造ルール

**plan.md は必ずサブディレクトリに保存すること。`_longruns/` 直下に置いてはならない。**

```
✅ 正しい: _longruns/2026-03-26_feature-name/plan.md
❌ 間違い: _longruns/plan.md
```
...
8. 日付付きサブディレクトリ `_longruns/YYYY-MM-DD_slug/plan.md` に保存する
9. ユーザーにレビューを依頼する

完了後、`/longrun:exec` コマンドで自律実行を開始できることを案内してください。
