...
このプラグインを使用するには、以下の Obsidian Vault 構造・設定が必要です。

### 1. Obsidian Vaultがgitリポジトリであること

Obsidian Vault 全体が `git init` されており、定期的にコミットされていること（例: obsidian-git プラグインによる自動バックアップ）。LLMセッションログやObsidianファイル変更の収集にVaultのgit履歴を使用します。
...
- **Obsidianファイル変更追跡**: Obsidian Vault gitログからプロジェクト内の変更ファイルを収集
