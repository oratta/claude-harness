...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: 'fetch_messages',
      description:
        "Fetch recent messages from a Discord channel. Returns oldest-first with message IDs and reactions (as [reactions: emoji×count]) — use this to check for 👍-style approvals on your own messages. Discord's search API isn't exposed to bots, so this is the only way to look back.",
      inputSchema: {
        type: 'object',
        properties: {
          channel: { type: 'string' },
          ...
          arr.length === 0
            ...
            : arr
                .map(m => {
                  ...
                  // Reaction emoji names are server-controlled (custom emoji)
                  // — strip the same delimiter chars as message content.
                  const reacts =
                    m.reactions.cache.size > 0
                      ? ` [reactions: ${[...m.reactions.cache.values()]
                          .map(r => `${(r.emoji.name ?? '?').replace(/[\r\n]/g, '')}×${r.count}`)
                          .join(' ')}]`
                      : ''
                  return `[${m.createdAt.toISOString()}] ${who}: ${text}${reacts}  (id: ${m.id}${atts})`
                })
                .join('\n')
