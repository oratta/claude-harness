...
- 命名規則: `support+<slug>@<domain>`（`<slug>` はプロダクト名から機械的に導出）
- ドメインの Email Routing で catch-all が有効か確認する（`+` サフィックスは多くの Email Routing サービスで完全一致解決されず個別ルールが必要になるため、catch-all → 集約先 Gmail への転送を前提にする）
