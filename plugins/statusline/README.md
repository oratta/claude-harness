...
- **それ以外のアカウント**は行頭が半角スペース 2 つになり、snapshot の値で描き、行末に取得からの経過時間（`2h前`）を添える。こちらには鮮度ゲートを効かせない。使っていないアカウントは OAuth トークンが期限切れで更新できないのが普通で、ゲートを効かせると 6 時間後に行が消えてしまうため
