...
| `label` | 左端に出す短いラベル。省略すると `id` を使う |
| `securestorage` | そのアカウントの `CLAUDE_SECURESTORAGE_CONFIG_DIR` の値。`null` / 空文字は既定アカウント |

トップレベルは必ずオブジェクト（裸の配列は不正）。`id` を欠くスロット・重複した `id`・9 個目以降のスロットは黙って捨てる。ファイル全体が壊れていれば既定アカウント 1 つに縮退するだけで、ステータスラインは止まらない。
...
いま使っているアカウントの判定は `CLAUDE_SECURESTORAGE_CONFIG_DIR` を優先し、取れなければ snapshot が記録している値にフォールバックする。ステータスラインのサブプロセスに環境変数が渡るかは Claude Code 側の実装に依存するため、両方に対応している。
