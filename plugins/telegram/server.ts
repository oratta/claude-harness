...
// Reaction updates (Bot API 7.0+, delivered because bot.start opts into
// message_reaction). Reactions can't go through handleInbound: there is no
// meaningful way to answer a reaction with a pairing prompt, so instead of
// gate() we drop anything not already allowlisted.
bot.on('message_reaction', ctx => {
  const access = loadAccess()
  if (access.dmPolicy === 'disabled') return
  const user = ctx.messageReaction.user
  if (!user || !access.allowFrom.includes(String(user.id))) return

  const { emojiAdded, emojiRemoved } = ctx.reactions()
  if (emojiAdded.length === 0 && emojiRemoved.length === 0) return
  const parts = [...emojiAdded.map(e => `+${e}`), ...emojiRemoved.map(e => `-${e}`)]

  mcp.notification({
    method: 'notifications/claude/channel',
    params: {
      content: `(reaction) ${parts.join(' ')}`,
      meta: {
        chat_id: String(ctx.messageReaction.chat.id),
        // The message the reaction was placed on — not a new message id.
        message_id: String(ctx.messageReaction.message_id),
...
void (async () => {
  for (let attempt = 1; ; attempt++) {
    try {
      await bot.start({
        // getUpdates excludes message_reaction unless asked for explicitly —
        // and once allowed_updates is passed, only the listed types arrive,
        // so every update type the handlers above rely on must be repeated.
