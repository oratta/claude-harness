...
| `scripts/casting-check.sh` | 壊れた表行・語彙 lint ＋起案シグナル検出（6項目）と `resolve`（有効配役表の合成。詳細は SKILL.md） |
...
`resolve` は出力前に project.md / local.md へ check モードと同じ検証（行形式・語彙・catalog_version）を通す。検証を通らない配役表では合成表を出力せず、理由を stderr に出して exit 1 する（fail-closed）。
