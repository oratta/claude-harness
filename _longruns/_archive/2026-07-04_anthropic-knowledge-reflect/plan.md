...
- **スコープ**: 新プラグイン `plugins/loops/` を作る。
  ...
  6. **`/loops:goalify <テキスト|ファイルパス>`（ゴールベース特化の一発コマンド）**: ユーザーの頻出ワークフロー「やりたいことを書き出す → /goal に読ませるファイルを作らせる → 不足情報はヒアリングさせる」を 1 コマンド化する。動き: (a) brain dump を読み、成功基準の機械検証可能化（コマンド + 期待値への変換）・停止条件（最大試行数/時間）・スコープ境界（触ってよい範囲/やらないこと）・前提（参照パス・環境）のうち**不足しているものだけ** AskUserQuestion でヒアリング（書き出しに既にある項目は聞かない。longrun-plan のインタビュー方法論を流用）、(b) `goals/<name>.goal.md`（目的 / 成功基準 [コマンド+期待値] / 制約 / 参照パス / エスカレーション条件）と、それを参照する /goal 起動コマンド 1 行を生成する。生成物は使い捨ての goal ブリーフであり、反復利用が見えたら recipe への昇格を促す 1 行を出力に含める（recipe-miner の検出対象とも整合）
...
**機能固有の条件:**
5. [ ] `plugins/loops/` が存在し、`/loops:design`・`/loops:goalify` スキル・`references/loop-types.md`（実行機構との責務分離の節を含む）・State テンプレート・レシピ形式規約を持つ
5b. [ ] `/loops:goalify` のデモ: brain dump テキストを与えて実行 → 不足情報のみのヒアリング → `goals/<name>.goal.md`（成功基準が全てコマンド+期待値で機械検証可能）と /goal 起動コマンド 1 行が生成される。書き出しに全情報が揃っている入力ではヒアリングが 0 問であることも確認
...
- 確認手順:
  ...
  2b. **goalify デモ**: やりたいことの書き出しテキストから `goals/*.goal.md` + /goal コマンドを生成し、実際に /goal で 1 回実行して成功基準の判定が機能することを確認
...
>
> （フィードバック6）「やりたいことを書き出す → goal コマンドに読ませるファイルを作って → 足りない情報はヒアリングして」をいつも手で指示している。これを一発で作るコマンドが欲しい → `/loops:goalify` として change-1 に追加
>
